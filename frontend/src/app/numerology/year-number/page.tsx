...
export default function PersonalYearPage() {
  ...
  const key = result
    ? `personal_year:${result.input?.birth_day}-${result.input?.birth_month}-${result.input?.year}`
    : "";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setResult(null);
    try {
      const data = await postJson<NumerologyCalcResult>("/numerology/personal-year", {
        birth_day: birthDay,
      ...
      console.error("Failed to calculate personal year", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggleSave = async () => {
    ...
    try {
      const k = `personal_year:${result.input?.birth_day}-${result.input?.birth_month}-${result.input?.year}`;
      const payload = {
  ...
  if (result) {
    ...
    return (
      <ProductPageScreen
        testId="numerology-year-number-page"
        title="Личный год"
        hideHeader
        ...
        <section style={{ padding: "var(--orbit-space-4xl) var(--orbit-space-xl) var(--orbit-space-2xl)", textAlign: "center" }}>
          <div style={{ maxWidth: "640px", margin: "0 auto" }}>
            <h1 className="orbit-display" style={{ fontSize: "clamp(2rem, 4vw, 3rem)", marginBottom: "var(--orbit-space-md)", color: "#0f172a", fontWeight: 500 }}>
              Личный год {String(result.input?.year ?? "")}
            </h1>
          </div>
        </section>
  ...
  return (
    <ProductPageScreen
      testId="numerology-year-number-page"
      title="Личный год"
      subtitle="День + месяц рождения + выбранный год → редукция, мастер-числа 11, 22, 33"
      contentClassName={`${pl.content} ${pl.legacyHost}`}
    >
      <section style={{ padding: "0 var(--orbit-space-xl) var(--orbit-space-4xl)" }}>
        <div style={{ maxWidth: "600px", margin: "0 auto" }}>
          <form onSubmit={handleSubmit} className="orbit-card" style={{ padding: "var(--orbit-space-xl)" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: "var(--orbit-space-lg)" }}>
              ...
              <div>
                <label className="orbit-body" style={{ display: "block", marginBottom: "var(--orbit-space-sm)", fontWeight: 500 }}>Месяц (1–12)</label>
                <select
                  value={birthMonth}
              ...
              <div>
                <label className="orbit-body" style={{ display: "block", marginBottom: "var(--orbit-space-sm)", fontWeight: 500 }}>Год</label>
                <select
                  value={year}

...
const SECTION_CONFIG = [
  {
    key: "love",
    titleKey: "catalog.relationships.section.love.title",
    descriptionKey: "catalog.relationships.section.love",
    productIds: ["compatibility_portal", "tarot_relationship", "astroclick_partners"]
  },
  {
    key: "family",
    titleKey: "catalog.relationships.section.family.title",
    descriptionKey: "catalog.relationships.section.family",
    productIds: ["child_profile", "parenting_checkin"]
  },
  {
    key: "integration",
    titleKey: "catalog.relationships.section.integration.title",
    descriptionKey: "catalog.relationships.section.integration",
    productIds: ["full_report", "numerology_deep_dive", "name_resonance"]
  }
];

export default function RelationshipCatalogPage() {
  return (
        ...
        <div className="orbit-hero-design-wrapper">
          ...
          <div className="orbit-hero-design-container">
            <p className="orbit-hero-design-subtitle" style={{ textAlign: "center", marginBottom: "var(--orbit-space-sm)", color: "rgba(255, 255, 255, 0.85)" }}>
              {t("catalog.relationships.hero.eyebrow", "Отношения")}
            </p>
            <h1 className="orbit-hero-design-title-text">{t("catalog.relationships.hero.title", "Сервисы про отношения")}</h1>
            <p className="orbit-hero-design-subtitle" style={{ textAlign: "center", marginTop: "var(--orbit-space-sm)" }}>
              {t("catalog.relationships.hero.body", "Совместимость, динамика пары, семейные связи и способы глубже понять важные отношения.")}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="orbit-hero-content-block">
        <div className="orbit-hero-content-container">
          ...
          <section className="orbit-card" style={{ background: "rgba(255, 255, 255, 0.95)", boxShadow: "0 4px 16px rgba(0, 0, 0, 0.08)" }}>
            <h2 className="orbit-display-sm">{t("catalog.relationships.recommendations.title", "Рекомендуемые сервисы")}</h2>
            <p className="orbit-body-sm orbit-text-muted">{t("catalog.relationships.recommendations.body", "Лучшие точки входа, если хочешь посмотреть на отношения через астрологию и смежные практики.")}</p>
            <CatalogRecommendationGrid slug="relationships" />
          </section>
          ...
          {/* Callout */}
          <CatalogCallout
            eyebrow={t("catalog.relationships.hero.eyebrow", "Отношения")}
            title={t("catalog.aside.relationships.title", "Куда смотреть дальше")}
            body={t("catalog.aside.relationships.body", "Если нужен другой угол разбора, открой соседние сервисы про пару, семью и совместимость.")}
          />
        </div>
      </section>
    </main>

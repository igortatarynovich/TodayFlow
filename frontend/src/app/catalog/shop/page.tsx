...
const SECTION_CONFIG = [
  {
    key: "bundles",
    titleKey: "catalog.shop.section.bundles.title",
    descriptionKey: "catalog.shop.section.bundles",
    productIds: ["bundle_best_of", "bundle_year_ahead"]
  },
  {
    key: "reports",
    titleKey: "catalog.shop.section.reports.title",
    descriptionKey: "catalog.shop.section.reports",
    productIds: ["eclipse_report", "second_half_life", "year_transits"]
  },
  {
    key: "gifts",
    titleKey: "catalog.shop.section.gifts.title",
    descriptionKey: "catalog.shop.section.gifts",
    productIds: ["gift_card", "poster_zodiac", "poster_aspects"]
  }
];
        ...
        <div className="orbit-hero-design-wrapper">
          ...
          <div className="orbit-hero-design-container">
            <p className="orbit-hero-design-subtitle" style={{ textAlign: "center", marginBottom: "var(--orbit-space-sm)", color: "rgba(255, 255, 255, 0.85)" }}>
              {t("catalog.shop.hero.eyebrow", "Подборки")}
            </p>
            <h1 className="orbit-hero-design-title-text">{t("catalog.shop.hero.title", "Подборки и наборы")}</h1>
            <p className="orbit-hero-design-subtitle" style={{ textAlign: "center", marginTop: "var(--orbit-space-sm)" }}>
              {t("catalog.shop.hero.body", "Сервисы и материалы, собранные в готовые наборы под конкретную задачу или тему.")}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="orbit-hero-content-block">
        <div className="orbit-hero-content-container">
          ...
          <section className="orbit-card" style={{ background: "rgba(255, 255, 255, 0.95)", boxShadow: "0 4px 16px rgba(0, 0, 0, 0.08)" }}>
            <h2 className="orbit-display-sm">{t("catalog.shop.recommendations.title", "Рекомендуемые подборки")}</h2>
            <p className="orbit-body-sm orbit-text-muted">{t("catalog.shop.recommendations.body", "Готовые наборы сервисов, если нужен уже собранный маршрут.")}</p>
            <CatalogRecommendationGrid slug="shop" />
          </section>
          ...
          {/* Callout */}
          <CatalogCallout
            eyebrow={t("catalog.shop.hero.eyebrow", "Подборки")}
            title={t("catalog.aside.shop.title", "Посмотреть другие подборки")}
            body={t("catalog.aside.shop.body", "Открой соседние наборы, если хочешь собрать маршрут под другую тему или глубину разбора.")}
          />
        </div>
      </section>
    </main>

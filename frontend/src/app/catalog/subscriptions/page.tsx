...
const SECTION_CONFIG = [
  {
    key: "core",
    titleKey: "catalog.subscriptions.section.core.title",
    descriptionKey: "catalog.subscriptions.section.core",
    productIds: ["subscription_lite_plus", "subscription_pdf_library"]
  },
  {
    key: "signals",
    titleKey: "catalog.subscriptions.section.signals.title",
    descriptionKey: "catalog.subscriptions.section.signals",
    productIds: ["subscription_tarot_flow", "subscription_transit_alerts"]
  }
];

export default function SubscriptionsCatalogPage() {
  return (
      ...
      <section className="orbit-hero-design orbit-hero-design-unified">
        <div className="orbit-hero-design-wrapper">
          <Image
            ...
            alt="Подписки и регулярные сервисы"
            fill
            priority
          ...
          <div className="orbit-hero-design-container">
            <p className="orbit-hero-design-subtitle" style={{ textAlign: "center", marginBottom: "var(--orbit-space-sm)", color: "rgba(255, 255, 255, 0.85)" }}>
              {t("catalog.subscriptions.hero.eyebrow", "Подписки")}
            </p>
            <h1 className="orbit-hero-design-title-text">{t("catalog.subscriptions.hero.title", "Подписки и регулярные сервисы")}</h1>
            <p className="orbit-hero-design-subtitle" style={{ textAlign: "center", marginTop: "var(--orbit-space-sm)" }}>
              {t("catalog.subscriptions.hero.body", "Повторяющиеся форматы и регулярные обновления, если нужен не разовый разбор, а устойчивый ритм сопровождения.")}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="orbit-hero-content-block">
        <div className="orbit-hero-content-container">
          ...
          <section className="orbit-card" style={{ background: "rgba(255, 255, 255, 0.95)", boxShadow: "0 4px 16px rgba(0, 0, 0, 0.08)" }}>
            <h2 className="orbit-display-sm">{t("catalog.subscriptions.recommendations.title", "Рекомендуемые подписки")}</h2>
            <p className="orbit-body-sm orbit-text-muted">{t("catalog.subscriptions.recommendations.body", "Подписочные сервисы, которые поддерживают регулярный ритм практики.")}</p>
            <CatalogRecommendationGrid slug="subscriptions" />
          </section>
          ...
          {/* Callout */}
          <CatalogCallout
            eyebrow={t("catalog.subscriptions.hero.eyebrow", "Подписки")}
            title={t("catalog.aside.subscriptions.title", "Сравнить форматы")}
            body={t("catalog.aside.subscriptions.body", "Перейди к соседним подпискам, если хочешь подобрать частоту, глубину и формат сопровождения.")}
          />
        </div>
      </section>
    </main>

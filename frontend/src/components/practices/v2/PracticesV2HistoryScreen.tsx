...
import { MotionReveal, MotionSettle } from "@/design-system/motion";
...
export function PracticesV2HistoryScreen({
  ...
  return (
    <div className={`${v2.pageRoot} ${styles.pageRoot}`} data-testid="practices-v2-history">
      ...
      <MotionReveal className={styles.header}>
        <header>
      ...
      {progressSummary ? (
        <MotionReveal as="section" className={`${v2.surfacePanel} ${styles.panel}`} delayMs={MOTION.staggerMs}>
          ...
          <div className={styles.statsGrid}>
            {(
              [
                { label: copy.currentStreakLabel, value: `${currentStreakDays} ${copy.streakDaysSuffix}` },
                { label: locale === "ru" ? "Завершено" : "Completed", value: String(progressSummary.totalCompleted) },
                {
      ...
      <MotionReveal
        as="section"
        className={`${v2.surfacePanel} ${styles.panel}`}
        delayMs={progressSummary ? MOTION.staggerMs * 2 : MOTION.staggerMs}
      >

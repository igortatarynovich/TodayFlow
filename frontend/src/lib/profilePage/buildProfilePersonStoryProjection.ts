/** Project Profile Availability 3.1 revealed_slots into person-story chapters. */

import type { CoreProfile } from "@/lib/types";
import {
  PROFILE_SLOT_HELPS,
  PROFILE_SLOT_IDENTITY,
...
/** Top → bottom person-story order (matrix 3.1). */
export const PERSON_STORY_SLOT_ORDER = [
  PROFILE_SLOT_IDENTITY,
  PROFILE_SLOT_SUN,
...
export type PersonStoryChapterKind =
  | "identity"
  | "sun_facts"
  | "catalog"
  | "name_numerology"
...
export type PersonStoryChapter = {
  id: string;
  kind: PersonStoryChapterKind;
  label: string;
  /** Primary text line(s) for simple chapters */
  lines: string[];
  /** Structured payload for richer chapters */
  payload: unknown;
};

export type PersonStoryProjection = {
  hasMatrix: boolean;
  identityLine: string | null;
  chapters: PersonStoryChapter[];
...
function sunLines(bag: unknown): string[] {
  if (!bag || typeof bag !== "object") return [];
  const o = bag as Record<string, unknown>;
  const parts: string[] = [];
  if (o.sun_sign) parts.push(String(o.sun_sign));
  if (o.element) parts.push(String(o.element));
  if (o.life_path != null && o.life_path !== "") parts.push(`путь ${o.life_path}`);
  if (o.birthday_number != null && o.birthday_number !== "") {
    parts.push(`день ${o.birthday_number}`);
  }
...
function catalogLines(bag: unknown): string[] {
  ...
  const stoneLabels = stones
    .map((s) => {
      ...
      if (s && typeof s === "object" && "label" in s) return String((s as { label?: unknown }).label || "");
      return "";
    })
    .filter(Boolean);
...
function natalLines(bag: unknown): string[] {
  if (!bag || typeof bag !== "object") return [];
  const o = bag as Record<string, unknown>;
  const angles = o.angles && typeof o.angles === "object" ? (o.angles as Record<string, unknown>) : {};
  const lines: string[] = [];
  ...
  const houses = Array.isArray(o.houses) ? o.houses : [];
  if (houses.length) lines.push(`Дома: ${houses.length} куспидов`);
  return lines;
}
...
function kindFor(slotId: string): PersonStoryChapterKind {
  if (slotId === PROFILE_SLOT_IDENTITY) return "identity";
  if (slotId === PROFILE_SLOT_SUN) return "sun_facts";
...
function labelFor(slotId: string): string {
  if (slotId === PROFILE_SLOT_IDENTITY) return "Узнавание";
  if (slotId === PROFILE_SLOT_SUN) return "Солнце · стихия · число";
...
function linesFor(slotId: string, value: unknown): string[] {
  if (slotId === PROFILE_SLOT_SUN) return sunLines(value);
...
/**
 * Build person-story chapters from revealed_slots only.
 * Empty bags omitted. Access-gated slots are not in revealed_slots.
 */
export function buildProfilePersonStoryProjection(
  core: CoreProfile | null | undefined,
): PersonStoryProjection {
  ...
  const chapters: PersonStoryChapter[] = [];
  for (const slotId of PERSON_STORY_SLOT_ORDER) {
    if (!(slotId in revealed)) continue;
    const value = revealed[slotId];
    if (isEmpty(value)) continue;
    const lines = linesFor(slotId, value);
    if (!lines.length && slotId !== PROFILE_SLOT_CATALOG && slotId !== PROFILE_SLOT_NATAL) {
      continue;
    }
    chapters.push({
      id: slotId,
  ...
  const identity = chapters.find((c) => c.kind === "identity");
  return {
    hasMatrix: true,
    identityLine: identity?.lines[0] ?? null,
    chapters,
    helpsLine: profileHelpsLineFromMatrix(core),
    userMessages,
    accessGatedHelps,
  };
}

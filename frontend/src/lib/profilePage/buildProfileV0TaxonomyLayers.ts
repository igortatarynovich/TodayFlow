...
export function buildProfileV0TaxonomySlots(input: BuildTaxonomyInput): ProfileV0TaxonomyPayload {
  ...
  const identity = trim(contract?.identity_core) || trim(interp?.identity);
  const strengths = trimList(contract?.strengths?.length ? contract.strengths : interp?.strengths, 3);
  const watchouts = trimList(
    contract?.growth_zones?.length ? contract.growth_zones : interp?.watchouts,
    2,
  );
  ...
  const identitySource = trim(contract?.identity_core)
    ? "profile_contract_v1.identity_core"
    : identity
      ? "interpretation.identity"
      : "life_path.essence";
  const identityKind = trim(contract?.identity_core) || trim(interp?.identity) ? "llm" : "reference";

  claim(reg, "hero", "identity", identityRaw, identitySource, identityKind, PROFILE_LIMITS.heroTagline);
  claim(reg, "hero", "main_strength", strengths[0] ? `Сильная сторона — ${strengths[0]}.` : lpEntry?.plus_side?.[0] ? `Сильная сторона — ${lpEntry.plus_side[0]}.` : sign?.strengths?.[0] ? `Сильная сторона — ${sign.strengths[0]}.` : null, strengths[0] ? (contract?.strengths?.length ? "profile_contract_v1.strengths[0]" : "interpretation.strengths[0]") : "life_path.plus_side[0]", strengths[0] ? "llm" : "reference", PROFILE_LIMITS.sphereLine);
  claim(reg, "hero", "main_conflict", watchouts[0] ? `Напряжение — ${watchouts[0]}.` : lpEntry?.minus_side?.[0] ? `Риск — ${lpEntry.minus_side[0]}.` : lpEntry?.main_fear ? `Страх — ${firstSentence(lpEntry.main_fear)}` : null, watchouts[0] ? (contract?.growth_zones?.length ? "profile_contract_v1.growth_zones[0]" : "interpretation.watchouts[0]") : "life_path.minus_side[0]", watchouts[0] ? "llm" : "reference", PROFILE_LIMITS.sphereLine);
  ...
  const decisionsText = trim(contract?.decision_style) || lifeAreas?.decisions || sign?.decisions || lpEntry?.reading?.[0] || null;
  claim(reg, "corePattern", "decisions", decisionsText, trim(contract?.decision_style) ? "profile_contract_v1.decision_style" : lifeAreas?.decisions ? "interpretation.life_areas.decisions" : "sign.decisions", trim(contract?.decision_style) || lifeAreas?.decisions ? "llm" : "reference", PROFILE_LIMITS.numbersInsight);
  ...
  const loveText = trim(contract?.relationship_style) || lifeAreas?.love || null;
  ...
  claim(reg, "love", "loves", loveText ? withRelationshipFrame(loveText) : sign?.love || sign?.conflict || null, trim(contract?.relationship_style) ? "profile_contract_v1.relationship_style" : lifeAreas?.love ? "interpretation.life_areas.love" : "sign.love", loveText ? "llm" : "reference", PROFILE_LIMITS.sphereMain);
  ...
  const moneyText = trim(contract?.money_style) || lifeAreas?.money || null;
  ...
  claim(reg, "money", "earning_style", moneyText ? withRealizationFrame(moneyText) : sign?.money || sign?.work || null, trim(contract?.money_style) ? "profile_contract_v1.money_style" : lifeAreas?.money ? "interpretation.life_areas.money" : "sign.money", moneyText ? "llm" : "reference", PROFILE_LIMITS.sphereMain);
  ...
  const checkSoon = trim(contract?.money_style) || lifeAreas?.career || null;
  claim(reg, "compass", "check_soon", checkSoon ? `Проверь: ${firstSentence(checkSoon)}` : moonRecoveryHint ? `Проверь: ${firstSentence(moonRecoveryHint)}` : lifeAreas?.body || null, trim(contract?.money_style) ? "profile_contract_v1.money_style" : lifeAreas?.career ? "interpretation.life_areas.career" : "moon_recovery_hint", checkSoon || lifeAreas?.career ? "llm" : "computed", PROFILE_LIMITS.movementRecommendation);

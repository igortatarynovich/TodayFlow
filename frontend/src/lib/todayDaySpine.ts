...
/**
 * Number rhythm facets — mirror of BE `number_base_v1.base_meaning`
 * (DATA/reference/numerology/number_base_v1/numbers.json).
 * Masters in use: 11/22/33. 44 not live. No bogus "20" (never emitted by BE reduce).
 */
const NUMBER_RHYTHM_BY_VALUE: Record<string, string> = {
  "1": "Единица — старт и ясное «я начинаю»: один первый шаг важнее десяти замыслов.",
  "2": "Двойка — баланс и пара: сегодня сильнее договорённость, чем давление.",
  "3": "Тройка — рост через выражение: идеи оживают в разговоре, движении и контакте.",
  "4": "Четвёрка — структура и фундамент: одна надёжная опора ценнее пяти новых стартов.",
  "5": "Пятёрка — смена темпа: гибкость и готовность скорректировать план важнее жёсткой схемы.",
  "6": "Шестёрка — ответственность и тепло: забота и честный долг работают лучше жёсткого контроля.",
  "7": "Семёрка — пауза и глубина: ответы приходят через наблюдение, не через давление.",
  "8": "Восьмёрка — сила результата: движение вперёд там, где есть ясная договорённость и границы.",
  "9": "Девятка — закрытие цикла: полезно завершить одно, прежде чем открывать новое.",
  "11": "Одиннадцать — обострённая интуиция: первому честному импульсу можно доверять, проверяя его действием.",
  "13": "Тринадцать — урок дисциплины: результат требует труда без срезания углов.",
  "14": "Четырнадцать — урок умеренности: свобода без побега в хаос и зависимости.",
  "16": "Шестнадцать — урок смирения: слом ложной опоры эго открывает место для честной перестройки.",
  "19": "Девятнадцать — урок зрелой самостоятельности: сила без одинокого злоупотребления властью.",
  "22": "Двадцать два — большая форма через конкретный шаг: масштаб мысли, приземлённый в одно действие.",
  "33": "Тридцать три — зрелая забота: вести и поддерживать без растворения себя в чужой нужде.",
};
...
export function buildNumberRhythmFacet(
  ...
  // No silent generic archetype — unknown value outside number_base_v1.
  return null;
}
...
export function buildSkyInfluenceCards(input: {
  ...
  if (ritualComplete && input.numerologyValue && input.numerologyValue !== "—") {
    const chorusNumber =
      ...
      null;
    const preset = NUMBER_RHYTHM_BY_VALUE[input.numerologyValue];
    const story =

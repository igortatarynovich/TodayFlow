...
def _fake_llm_interp(pack, **_kwargs):
    ...
    # Mention question once only (quality gate).
    story = (
        f"Карты {named} складываются в один конфликт вокруг темы выбора и ясности: "
        f"позиции усиливают различие между импульсом и проверкой фактов."
    )
    if pack.get("spread_kind") == "choice":
        story = (
            f"Путь A через {names[0] if names else 'первую карту'} быстрее снимает туман, "
            f"но несёт риск спешки; путь B через {names[2] if len(names) > 2 else (names[-1] if names else 'другую карту')} "
            f"даёт прояснение, однако может удерживать в старом сценарии."
        )
    return {
        "symbols_overview": (
            f"Ключевые символы — {named}. Свет и тень мастей задают напряжение между движением и удержанием."
        ),
        "question_story": story,
        "direct_answer": (
            f"По сути вопроса нужен проверяемый шаг, а не идеальная гарантия: "

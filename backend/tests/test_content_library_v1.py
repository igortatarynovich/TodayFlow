...
SEED_29_ID = "practice.free_writing.001"
...
SEED_32_ID = "meditation.breath_awareness.001"
SEED_32_CELL = "need.presence.stabilize"
SEED_33_ID = "meditation.body_scan.001"
SEED_33_CELL = "need.self_connection.reflect"
SEED_34_ID = "meditation.open_awareness.001"
...
SEED_37_ID = "meditation.reflection_meditation.001"
SEED_37_CELL = "need.clarity.reflect"
SEED_38_ID = "affirmation.agency.001"
...
SEED_41_ID = "discipline.consumption_limit.001"
SEED_41_CELL = "need.simplicity.release"
...
def test_fill_unfrozen_provisional_probes() -> None:
    ...
    assert coverage["next_fill_cell"] is None
    ...
    assert "affirmation.self_worth" in coverage["skipped_types"]
    assert "affirmation.permission" in coverage["skipped_types"]
    ...
    assert sourced == {
        ...
        "practice.free_writing.001": "technique.free_writing",
        "practice.free_writing.002": "technique.free_writing",
        "practice.free_writing.003": "technique.free_writing",
        ...
        "meditation.breath_awareness.001": "technique.breath_awareness",
        "meditation.breath_awareness.002": "technique.breath_awareness",
        "meditation.breath_awareness.003": "technique.breath_awareness",
        "meditation.body_scan.001": "technique.body_scan",
        "meditation.body_scan.002": "technique.body_scan",
        "meditation.body_scan.003": "technique.body_scan",
        "meditation.open_awareness.001": "technique.open_awareness",
        "meditation.open_awareness.002": "technique.open_awareness",
        "meditation.open_awareness.003": "technique.open_awareness",
        ...
        "meditation.reflection_meditation.001": "technique.reflection_meditation",
        "meditation.reflection_meditation.002": "technique.reflection_meditation",
        "meditation.reflection_meditation.003": "technique.reflection_meditation",
        "affirmation.agency.001": "technique.agency",
        "affirmation.agency.002": "technique.agency",
        "affirmation.agency.003": "technique.agency",
        ...
        "discipline.consumption_limit.001": "technique.consumption_limit",
        "discipline.consumption_limit.002": "technique.consumption_limit",
        "discipline.consumption_limit.003": "technique.consumption_limit",
        "practice.paced_breathing.001": "technique.paced_breathing",
        "practice.physiological_sigh.001": "technique.physiological_sigh",
        "practice.grounding.001": "technique.practice_grounding",
        ...
        "discipline.consistency_challenge.001": "technique.consistency_challenge",
        "discipline.consistency_challenge.003": "technique.consistency_challenge",
        "discipline.consistency_challenge.004": "technique.consistency_challenge",
        "discipline.reduction.001": "technique.reduction",
        "discipline.reduction.002": "technique.reduction",
        "discipline.reduction.003": "technique.reduction",
        "practice.digital_pause.001": "technique.digital_pause",
        "practice.digital_pause.002": "technique.digital_pause",
        "practice.digital_pause.003": "technique.digital_pause",
        "meditation.mindfulness.001": "technique.mindfulness",
        "meditation.mindfulness.002": "technique.mindfulness",
        "meditation.mindfulness.003": "technique.mindfulness",
        "discipline.consistency_challenge.002": "technique.consistency_challenge",
        "practice.stretching.001": "technique.stretching",
        "practice.mindful_movement.001": "technique.mindful_movement",
        "practice.walking.001": "technique.walking",
        "practice.shaking_release.001": "technique.shaking_release",
        ...
        "practice.boundary_action.001": "technique.boundary_action",
        "practice.drawing.001": "technique.drawing",
...
def test_seed_pass_closes_exactly_one_cell_per_item() -> None:
    ...
    for iid, item in items.items():
        seed_cell = item["identity"].get("seed_cell")
        if seed_cell is None:
            continue
        assert listed.get(iid) == [seed_cell]
...
def test_p0_seed_22_consistency_cell_sourced() -> None:
    _vocab, library, coverage = _load()
    ...
    assert item["identity"]["technique_id"] == "technique.consistency_challenge"
    assert item["identity"]["status"] == "active"
...
def test_p0_seed_23_simplicity_cell_sourced() -> None:
    _vocab, library, coverage = _load()
    ...
    assert item["identity"]["technique_id"] == "technique.reduction"
    assert item["identity"]["status"] == "active"
...
def test_p0_seed_24_reset_cell_sourced() -> None:
    _vocab, library, coverage = _load()
    ...
    assert item["identity"]["technique_id"] == "technique.digital_pause"
    assert item["identity"]["status"] == "active"
...
def test_p0_seed_25_presence_cell_sourced() -> None:
    _vocab, library, coverage = _load()
    ...
    assert item["identity"]["technique_id"] == "technique.mindfulness"
    assert item["identity"]["status"] == "active"
...
def test_p0_seed_26_habit_change_cell_sourced() -> None:
    _vocab, library, coverage = _load()
...
def test_p1_paced_breathing_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "practice.paced_breathing.001")
    assert item["identity"]["content_class"] == "practice"
    assert item["identity"]["family"] == "breathwork"
    assert item["identity"]["type"] == "paced_breathing"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.paced_breathing"
    ...
    row = next(r for r in coverage["type_spine"] if r["content_class"] == "practice" and r["type"] == "paced_breathing")
    assert row["phase"] == "P1"
    assert "practice.paced_breathing.001" in row["item_ids"]


def test_p1_physiological_sigh_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "practice.physiological_sigh.001")
    assert item["identity"]["content_class"] == "practice"
    assert item["identity"]["family"] == "breathwork"
    assert item["identity"]["type"] == "physiological_sigh"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.physiological_sigh"
    assert "seed_cell" not in item["identity"]
    assert item["retrieval"]["purpose"] == ["calm"]
    assert item["retrieval"]["direction"] == ["downregulate"]
    assert item["retrieval"]["intensity"] == "low"
    ...
    assert str(item["payload"]["locales"]["en"]["title"]).strip()
    assert str(item["payload"]["locales"]["ru"]["title"]).strip()
    row = next(r for r in coverage["type_spine"] if r["content_class"] == "practice" and r["type"] == "physiological_sigh")
    assert row["phase"] == "P1"
    assert "practice.physiological_sigh.001" in row["item_ids"]


def test_p1_practice_grounding_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "practice.grounding.001")
    assert item["identity"]["content_class"] == "practice"
    assert item["identity"]["family"] == "somatic"
    assert item["identity"]["type"] == "grounding"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.practice_grounding"
    assert "seed_cell" not in item["identity"]
    assert item["retrieval"]["purpose"] == ["grounding"]
    assert item["retrieval"]["direction"] == ["stabilize"]
    assert item["retrieval"]["intensity"] == "low"
    assert item["payload"]["body_kind"] == "instruction"
    assert str(item["payload"]["locales"]["en"]["title"]).strip()
    assert str(item["payload"]["locales"]["ru"]["title"]).strip()
    row = next(r for r in coverage["type_spine"] if r["content_class"] == "practice" and r["type"] == "grounding")
    assert row["phase"] == "P1"
    assert "practice.grounding.001" in row["item_ids"]


def test_p1_stretching_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "practice.stretching.001")
    assert item["identity"]["content_class"] == "practice"
    assert item["identity"]["family"] == "movement"
    assert item["identity"]["type"] == "stretching"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.stretching"
    assert "seed_cell" not in item["identity"]
    assert item["retrieval"]["purpose"] == ["release"]
    assert item["retrieval"]["direction"] == ["release"]
    assert item["retrieval"]["intensity"] == "low"
    ...
    assert str(item["payload"]["locales"]["en"]["title"]).strip()
    assert str(item["payload"]["locales"]["ru"]["title"]).strip()
    row = next(r for r in coverage["type_spine"] if r["content_class"] == "practice" and r["type"] == "stretching")
    assert row["phase"] == "P1"
    assert "practice.stretching.001" in row["item_ids"]


def test_p1_mindful_movement_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "practice.mindful_movement.001")
    assert item["identity"]["content_class"] == "practice"
    assert item["identity"]["family"] == "movement"
    assert item["identity"]["type"] == "mindful_movement"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.mindful_movement"
    assert "seed_cell" not in item["identity"]
    assert item["retrieval"]["purpose"] == ["presence"]
    assert item["retrieval"]["direction"] == ["stabilize"]
    assert item["retrieval"]["intensity"] == "low"
    ...
    assert str(item["payload"]["locales"]["en"]["title"]).strip()
    assert str(item["payload"]["locales"]["ru"]["title"]).strip()
    row = next(r for r in coverage["type_spine"] if r["content_class"] == "practice" and r["type"] == "mindful_movement")
    assert row["phase"] == "P1"
    assert "practice.mindful_movement.001" in row["item_ids"]


def test_p1_walking_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "practice.walking.001")
    assert item["identity"]["content_class"] == "practice"
    assert item["identity"]["family"] == "movement"
    assert item["identity"]["type"] == "walking"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.walking"
    assert "seed_cell" not in item["identity"]
    assert item["retrieval"]["purpose"] == ["energy"]
    assert item["retrieval"]["direction"] == ["activate"]
    assert item["retrieval"]["intensity"] == "low"
    ...
    assert str(item["payload"]["locales"]["en"]["title"]).strip()
    assert str(item["payload"]["locales"]["ru"]["title"]).strip()
    row = next(r for r in coverage["type_spine"] if r["content_class"] == "practice" and r["type"] == "walking")
    assert row["phase"] == "P1"
    assert "practice.walking.001" in row["item_ids"]


def test_p1_shaking_release_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "practice.shaking_release.001")
    assert item["identity"]["content_class"] == "practice"
    assert item["identity"]["family"] == "movement"
    assert item["identity"]["type"] == "shaking_release"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.shaking_release"
    assert "seed_cell" not in item["identity"]
    assert item["retrieval"]["purpose"] == ["release"]
    assert item["retrieval"]["direction"] == ["release"]
    assert item["retrieval"]["intensity"] == "low"
    ...
    assert str(item["payload"]["locales"]["en"]["title"]).strip()
    assert str(item["payload"]["locales"]["ru"]["title"]).strip()
    row = next(r for r in coverage["type_spine"] if r["content_class"] == "practice" and r["type"] == "shaking_release")
    assert row["phase"] == "P1"
    assert "practice.shaking_release.001" in row["item_ids"]
    assert "practice.shaking_release" not in coverage["skipped_types"]


def test_p1_practice_gratitude_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "practice.gratitude.001")
    assert item["identity"]["content_class"] == "practice"
    assert item["identity"]["family"] == "reflection"
    assert item["identity"]["type"] == "gratitude"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.practice_gratitude"
    assert "seed_cell" not in item["identity"]
    assert item["retrieval"]["purpose"] == ["self_connection"]
    assert item["retrieval"]["direction"] == ["reflect"]
    assert item["retrieval"]["intensity"] == "low"
    ...
    assert str(item["payload"]["locales"]["en"]["title"]).strip()
    assert str(item["payload"]["locales"]["ru"]["title"]).strip()
    row = next(r for r in coverage["type_spine"] if r["content_class"] == "practice" and r["type"] == "gratitude")
    assert row["phase"] == "P1"
    assert "practice.gratitude.001" in row["item_ids"]


def test_p1_practice_review_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "practice.review.001")
    assert item["identity"]["content_class"] == "practice"
    assert item["identity"]["family"] == "reflection"
    assert item["identity"]["type"] == "review"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.review"
    assert "seed_cell" not in item["identity"]
    assert item["retrieval"]["purpose"] == ["clarity"]
    assert item["retrieval"]["direction"] == ["reflect"]
    assert item["retrieval"]["intensity"] == "low"
    ...
    assert str(item["payload"]["locales"]["en"]["title"]).strip()
    assert str(item["payload"]["locales"]["ru"]["title"]).strip()
    row = next(r for r in coverage["type_spine"] if r["content_class"] == "practice" and r["type"] == "review")
    assert row["phase"] == "P1"
    assert "practice.review.001" in row["item_ids"]


def test_p1_practice_visualization_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "practice.visualization.001")
    assert item["identity"]["content_class"] == "practice"
    assert item["identity"]["family"] == "intention"
    assert item["identity"]["type"] == "visualization"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.practice_visualization"
    assert "seed_cell" not in item["identity"]
    assert item["retrieval"]["purpose"] == ["confidence"]
    assert item["retrieval"]["direction"] == ["prepare"]
    assert item["retrieval"]["intensity"] == "low"
...
def test_p1_practice_boundary_action_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "practice.boundary_action.001")
    assert item["identity"]["content_class"] == "practice"
    assert item["identity"]["family"] == "behavioral"
    assert item["identity"]["type"] == "boundary_action"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.boundary_action"
    assert "seed_cell" not in item["identity"]
    assert item["retrieval"]["purpose"] == ["self_control"]
    assert item["retrieval"]["direction"] == ["stabilize"]
    assert item["retrieval"]["intensity"] == "low"
    ...
    assert str(item["payload"]["locales"]["en"]["title"]).strip()
    assert str(item["payload"]["locales"]["ru"]["title"]).strip()
    row = next(r for r in coverage["type_spine"] if r["content_class"] == "practice" and r["type"] == "boundary_action")
    assert row["phase"] == "P1"
    assert "practice.boundary_action.001" in row["item_ids"]


def test_p1_practice_drawing_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "practice.drawing.001")
    assert item["identity"]["content_class"] == "practice"
    assert item["identity"]["family"] == "creative"
    assert item["identity"]["type"] == "drawing"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.drawing"
    assert "seed_cell" not in item["identity"]
    assert item["retrieval"]["purpose"] == ["creativity"]
    assert item["retrieval"]["direction"] == ["open"]
    assert item["retrieval"]["intensity"] == "low"
    ...
    assert str(item["payload"]["locales"]["en"]["title"]).strip()
    assert str(item["payload"]["locales"]["ru"]["title"]).strip()
    assert "drawing" not in str(item["payload"]).lower()
    row = next(r for r in coverage["type_spine"] if r["content_class"] == "practice" and r["type"] == "drawing")
    assert row["phase"] == "P1"
    assert "practice.drawing.001" in row["item_ids"]


def test_p1_practice_music_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "practice.music.001")
    assert item["identity"]["content_class"] == "practice"
    assert item["identity"]["family"] == "creative"
    assert item["identity"]["type"] == "music"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.music"
    assert "seed_cell" not in item["identity"]
    assert item["retrieval"]["purpose"] == ["creativity"]
    assert item["retrieval"]["direction"] == ["open"]
    assert item["retrieval"]["intensity"] == "low"
    ...
    assert str(item["payload"]["locales"]["en"]["title"]).strip()
    assert str(item["payload"]["locales"]["ru"]["title"]).strip()
    assert "music" not in str(item["payload"]).lower()
    row = next(r for r in coverage["type_spine"] if r["content_class"] == "practice" and r["type"] == "music")
    assert row["phase"] == "P1"
    assert "practice.music.001" in row["item_ids"]


def test_p1_practice_release_ritual_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "practice.release_ritual.001")
    assert item["identity"]["content_class"] == "practice"
    assert item["identity"]["family"] == "ritual"
    assert item["identity"]["type"] == "release_ritual"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.release_ritual"
    assert "seed_cell" not in item["identity"]
    assert item["retrieval"]["purpose"] == ["release"]
    assert item["retrieval"]["direction"] == ["release"]
    assert item["retrieval"]["intensity"] == "low"
    ...
    assert str(item["payload"]["locales"]["en"]["title"]).strip()
    assert str(item["payload"]["locales"]["ru"]["title"]).strip()
    assert "release_ritual" not in str(item["payload"]).lower()
    row = next(r for r in coverage["type_spine"] if r["content_class"] == "practice" and r["type"] == "release_ritual")
    assert row["phase"] == "P1"
    assert "practice.release_ritual.001" in row["item_ids"]


def test_p1_meditation_visualization_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "meditation.visualization.001")
    assert item["identity"]["content_class"] == "meditation"
    assert item["identity"]["type"] == "visualization"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.visualization"
    assert "seed_cell" not in item["identity"]
    assert item["retrieval"]["purpose"] == ["calm"]
    assert item["retrieval"]["direction"] == ["downregulate"]
    assert item["retrieval"]["intensity"] == "low"
    ...
    assert str(item["payload"]["locales"]["en"]["title"]).strip()
    assert str(item["payload"]["locales"]["ru"]["title"]).strip()
    assert "visualization" not in str(item["payload"]).lower()
...
def test_p1_meditation_gratitude_sourced() -> None:
    _vocab, library, coverage = _load()
    item = next(i for i in library["items"] if i["identity"]["item_id"] == "meditation.gratitude.001")
    assert item["identity"]["content_class"] == "meditation"
    assert item["identity"]["type"] == "gratitude"
    assert item["identity"]["status"] == "active"
    assert item["identity"]["technique_id"] == "technique.gratitude"
    assert "seed_cell" not in item["identity"]
    assert item["retrieval"]["purpose"] == ["presence"]
    assert item["retrieval"]["direction"] == ["stabilize"]
    assert item["retrieval"]["intensity"] == "low"
    ...
    assert str(item["payload"]["locales"]["en"]["title"]).strip()
    assert str(item["payload"]["locales"]["ru"]["title"]).strip()
    assert "gratitude" not in str(item["payload"]).lower()
...
def test_p1_meditation_silence_skipped() -> None:
    _vocab, library, coverage = _load()
    techniques = _techniques()
    row = next(t for t in techniques["techniques"] if t["technique_id"] == "technique.silence")
    assert row["content_class"] == "meditation"
    assert row["type"] == "silence"
    assert row["status"] == "skipped"
    assert row["skip_reason"] == "family_collapse"
    assert "meditation.silence" in coverage["skipped_types"]
    spine = next(r for r in coverage["type_spine"] if r["content_class"] == "meditation" and r["type"] == "silence")
...
def test_p1_affirmation_self_worth_skipped() -> None:
    _vocab, library, coverage = _load()
    techniques = _techniques()
    row = next(t for t in techniques["techniques"] if t["technique_id"] == "technique.self_worth")
    assert row["content_class"] == "affirmation"
    assert row["type"] == "self_worth"
    assert row["status"] == "skipped"
    assert row["skip_reason"] == "source_gap"
    assert "affirmation.self_worth" in coverage["skipped_types"]
    spine = next(r for r in coverage["type_spine"] if r["content_class"] == "affirmation" and r["type"] == "self_worth")
    assert spine["phase"] == "P1"
    assert spine["item_ids"] == []
    assert not any(i["identity"].get("type") == "self_worth" for i in library["items"])
    assert not any(i["identity"].get("technique_id") == "technique.self_worth" for i in library["items"])


def test_p1_affirmation_permission_skipped() -> None:
    _vocab, library, coverage = _load()
    techniques = _techniques()
    row = next(t for t in techniques["techniques"] if t["technique_id"] == "technique.permission")
    assert row["content_class"] == "affirmation"
    assert row["type"] == "permission"
    assert row["status"] == "skipped"
    assert row["skip_reason"] == "source_gap"
    assert "affirmation.permission" in coverage["skipped_types"]
    spine = next(r for r in coverage["type_spine"] if r["content_class"] == "affirmation" and r["type"] == "permission")
    assert spine["phase"] == "P1"
    assert spine["item_ids"] == []
    assert not any(i["identity"].get("type") == "permission" for i in library["items"])
    assert not any(i["identity"].get("technique_id") == "technique.permission" for i in library["items"])

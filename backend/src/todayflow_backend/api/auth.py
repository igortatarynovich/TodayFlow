...
from todayflow_backend.services.email import send_password_reset_email, send_welcome_email
...
def email_signup(payload: EmailSignupPayload, request: Request, db: Session = Depends(get_session)) -> dict:
    ...
    magic_token_str = db_models.PasswordResetToken.generate_token()
    expires_at = utc_naive_now() + timedelta(hours=72)
    db.add(
...
def magic_login(payload: MagicLoginPayload, request: Request, db: Session = Depends(get_session)) -> dict:
    """One-click login from welcome email."""
    locale = request_locale(request)
    token_record = (
    ...
    if not token_record or not token_record.is_valid():
        raise HTTPException(status_code=400, detail=translate("auth.errors.invalidToken", locale=locale))

    user = db.query(db_models.User).filter_by(id=token_record.user_id).first()
    if not user:
        raise HTTPException(status_code=400, detail=translate("auth.errors.invalidToken", locale=locale))

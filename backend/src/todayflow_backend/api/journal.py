...
from todayflow_backend.db.models import JournalEntry, User, utc_naive_now
...
def get_journal_repetitions(
    ...
    """
    seven_days_ago = utc_naive_now() - timedelta(days=7)
...
def export_journal_csv(
    ...
    # Generate filename
    filename = f"journal_entries_{utc_naive_now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def export_journal_json(
    ...
    # Generate filename
    filename = f"journal_entries_{utc_naive_now().strftime('%Y%m%d_%H%M%S')}.json"

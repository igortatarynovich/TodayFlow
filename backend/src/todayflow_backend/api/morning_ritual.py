...
async def get_morning_ritual(
    ...
    if isinstance(daily_recommendations, dict):
    ...
    celestial_events = await build_celestial_events(
        target_date_obj,
        locale,
        personal_day=int(personal_day_value) if personal_day_value is not None else None,
...
async def _get_daily_horoscope(
    ...
    if cached_generation is not None:
        ...
        return sanitize_daily_horoscope_payload(cached_payload), cached_generation.id
...
def _spine_best_mode(
    *,
    ...
    """Human day headline for contract period — not an engine template."""
    summary = str(day_summary or "").strip()
    if summary and len(summary) >= 20 and not summary.lower().startswith("проживать день"):
        return summary if summary[-1] in ".!?" else f"{summary}."

    do_clean = str(do_focus or "один понятный шаг").strip().rstrip(".")
    element = str(element_focus or "ясный фокус").strip()
    label = str(focus_label_ru or "").strip()
    if label and label.lower() not in {"general", "общее", "общий фокус"}:
        return f"Сегодня — {label.lower()}: {do_clean.lower()}."
    return f"Сегодня важнее {element.lower()}: {do_clean.lower()}."
...
def _build_deterministic_spine(
    ...
    return {
        ...
        "best_mode": _spine_best_mode(
            day_summary=day_summary,
            element_focus=str(baseline.get("element_focus") or ""),
            do_focus=str(do_focus),
            focus_label_ru=focus_label_ru,
        ),

...
from todayflow_backend.services.guide_flow_signals import guide_meaning_completions_today_counts
from todayflow_backend.services.fusion_scores import build_fusion_scores_for_inputs
...
class FusionIndexResponse(BaseModel):
    ...
    encouragement: str
    # DE-9 + O7: вчера + 7 дней + дельта (тот же контракт, что в DayContext.layers.history).
    day_history: Optional[Dict[str, Any]] = Field(
        default=None,
        description="day_history_v0 для UI «ритм / вчера»; не путать с scores за выбранный день. "
        "O7: при отсутствии вчерашних отметок Flow — fusion_score_delta_trustworthy=false (не показывать дельту).",
    )
...
def checkin_ascetic_contract(
    ...
    if existing_progress:
        ...
        existing_progress.updated_at = utc_naive_now()
    else:
        db.add(
            ProgressTrackerEntry(
                user_id=current_user.id,
    ...
    contract.updated_at = utc_naive_now()
    db.commit()
    db.refresh(contract)
    return AsceticContractResponse(
        id=contract.id,
...
def get_daily_fusion_index(
    ...
    guide_action_options_selected_today = int(
        db.query(func.count(MeaningEvent.id))
        .filter(
            MeaningEvent.user_id == current_user.id,
            MeaningEvent.local_date == day,
            MeaningEvent.event_type == "action_option_selected",
        )
        .scalar()
        or 0
    )

    # DE-7 v2: явные «сделано» из meaning_events (привычка / практика / фокус / аффирмация / аскеза).
    # Дополняют флаг `day_completed` в DayConnection (этап «Шаг дня» в UI), а не заменяют его.
    guide_meaning_completions_today = guide_meaning_completions_today_counts(
    ...
    scores = build_fusion_scores_for_inputs(
    ...
    day_history = build_history_layer_v0(
        db,
        user_id=current_user.id,
        target_date=day,
        today_fusion_scores=scores,
    )
    ...
    return FusionIndexResponse(
        ...
        activity_context={
            ...
            # DE-7: этапы связки дня → fusion / DayContext / narrative (сервер всегда тянет из БД в POST /today/narrative).
            "morning_completed": bool(day_connection.morning_completed) if day_connection else False,
            "day_completed": bool(day_connection.day_completed) if day_connection else False,
            "evening_completed": bool(day_connection.evening_completed) if day_connection else False,
            "guide_action_options_selected_today": guide_action_options_selected_today,
            "guide_meaning_completions_today": guide_meaning_completions_today,
        },
...
def step_weekly_goal(
    ...
    step_date = datetime.strptime(payload.date, "%Y-%m-%d").date() if payload.date else datetime.now(timezone.utc).date()

...
PROMPT_VERSION = "profile-interpreter-v8"

PROFILE_INTERPRETER_SYSTEM_PROMPT = f"""Ты пишешь личную интерпретацию профиля для TodayFlow.
...
- добавить ещё пять коротких сфер (каждая — один абзац, без дословного повторения базовых четырёх): сексуальность (прямо про желание, тело, инициативу, границы и связь в сексе), дети и родительство (нейтрально, если тема может быть неактуальна), тело и энергия, дружба и окружение, решения и дисциплина.
- отдельно sex_practical_tips: 3–4 коротких практических подсказки — позы/ритм, фразы для старта или паузы, что делать при отказе или разном желании
...
{{
  "interpretation": {{
    ...
    "strengths": ["...", "...", "..."],
    "watchouts": ["...", "...", "..."],
    "sex_practical_tips": ["3–4 практических подсказки про секс и желание"],
    "life_areas": {{
      "love": "...",
      "career": "...",
      "money": "...",
    ...
    return {
        "interpretation": {
            ...
            "sex_practical_tips": [
                ...
                "После отказа партнёра не проверяй «ещё раз» в тот же вечер — спроси, когда вернуться к теме, и переключись на не-сексуальную близость.",
            ],
            "life_areas": {
                ...
                "sex": "В сексе и желании тебе обычно важны честность, ясный темп и право говорить «да» и «нет» без игры; когда притяжение есть, но нет слов, тело часто берёт на себя то, что не проговорено — отсюда и вспышки, и откаты.",
...
def _strip_meta_from_interpretation(interpretation: dict[str, Any]) -> None:
    identity = interpretation.get("identity")
    if isinstance(identity, str):
        interpretation["identity"] = strip_meta_editorial_phrases(identity)
    for key in ("strengths", "watchouts", "sex_practical_tips"):
        xs = interpretation.get(key)
        if isinstance(xs, list):
            interpretation[key] = [
                strip_meta_editorial_phrases(str(x)) for x in xs if str(x).strip()
            ]
    la = interpretation.get("life_areas")
    if isinstance(la, dict):
        for lk, lv in list(la.items()):
            if isinstance(lv, str):
                la[lk] = strip_meta_editorial_phrases(lv)


def _sanitize_sex_practical_tips(interpretation: dict[str, Any]) -> None:
    raw = interpretation.get("sex_practical_tips")
    if raw is None:
        return
    if not isinstance(raw, list):
        interpretation.pop("sex_practical_tips", None)
        return
    tips = [str(x).strip() for x in raw if str(x).strip()]
    tips = [t for t in tips if len(t.split()) >= 4][:5]
    if len(tips) >= 2:
        interpretation["sex_practical_tips"] = tips
    else:
        interpretation.pop("sex_practical_tips", None)
...
def _normalize_profile_interpretation_shape(data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if isinstance(interpretation, dict) and isinstance(daily_interpretation, dict):
        ...
        _sanitize_sex_practical_tips(interpretation)
        _strip_meta_from_interpretation(interpretation)
        normalized = {
    ...
    if legacy_keys.issubset(data.keys()):
        ...
        if isinstance(data.get("sex_practical_tips"), list):
            legacy_interp["sex_practical_tips"] = data.get("sex_practical_tips")
        _sanitize_sex_practical_tips(legacy_interp)
        _strip_meta_from_interpretation(legacy_interp)
        normalized = {

...
def build_guide_decision_v0(
    *,
    ...
    if en:
        ...
        ht_line = ritual_head_topic_line_ru(head_topic)
        if ht_line:
            conflict = f"{conflict} Тема «в голове»: {ht_line} — учитывай её в выборе одной линии, не расползайся."
    ...
    if en:
        ...
        if card and low:
            ...
            # Совпадает по лексике с дефолтным anchor_summary day_narrative_brief_v0 (пустой ритуал):
            # иначе P0-alignment headline+subline с brief ломается после подмены guide_decision.
            headline = _clip(
                "Ось дня из гороскопа — один завершённый шаг без лишнего героизма.",
                130,
            )
        subline = _clip(

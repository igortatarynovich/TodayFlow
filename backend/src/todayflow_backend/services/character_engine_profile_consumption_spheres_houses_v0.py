...
# Personal planet in house — short accent (caller folds without «заметнее заметнее»).
_PLANET_FUNCTION: dict[str, str] = {
    "Sun": "воля",
    "Moon": "потребность в безопасности",
    "Mercury": "речь и договорённости",
    "Venus": "притяжение и оценка",
    "Mars": "инициатива и темп",
}

# Person-facing theses — one punch about the person (not encyclopedia / not day agenda).
_HOUSE_ZONE: dict[int, dict[str, str]] = {
    1: {
        "how": "Тебя считывают по темпу и дистанции ещё до того, как узнают ядро — первый жест задаёт всё.",
        "do": "В новом контакте зафиксируй свой первый жест: ближе, дальше или сразу правила.",
    },
    2: {
        "how": "Самооценка для тебя = «это моё»: деньги, навыки, право не оправдываться.",
        "do": "Назови одну вещь или навык, который не отдаёшь «ради мира».",
    },
    3: {
    ...
    4: {
        "how": "Дом либо заряжает тебя, либо становится клеткой — приватный ритм не декорация.",
        "do": "Сделай дома один завершённый жест восстановления с явным началом и концом.",
    },
    5: {
        "how": "Игра и риск «ради себя» показывают, умеешь ли ты жить не только ролью.",
    ...
    7: {
        "how": "Равная связь для тебя = отдельные решения без разрыва и без «мы вместо я».",
        "do": "Зафиксируй с близким одно правило двоих (время, деньги, доступ) — без намёков.",
    },
    8: {
        "how": "В совместных ресурсах и переменах контроль у тебя спорит с уязвимостью.",
        "do": "Назови одной фразой момент, где держишь контроль вместо прямого разговора.",
    },
    9: {
    ...
    10: {
        "how": "Публичная роль жива, только если видимость не требует сдать твой метод.",
        "do": "Выбери один видимый рабочий результат и доведи до проверяемого «сделано».",
    },
    11: {
        "how": "В сети важно, где взаимный вектор, а где только поддержка контакта.",
...
def _sign_tint_clause(sign: str | None) -> str:
    """Fold cusp modality into lived colour — no «Темп зоны» meta label."""
    if not sign:
        return ""
    modality = _SIGN_MODALITY.get(sign, "")
    if not modality:
        return ""
    return f" Здесь это звучит как {modality}."
...
def _planet_function_clause(planets: list[str]) -> str:
    ...
    if not bits:
        return ""
    return " Сильнее звучит " + ", ".join(bits) + "."
...
def apply_spheres_and_houses_to_payload(
    ...
    *,
    ...
    payload["character_engine_house_lines_v0"] = {
        "projection_version": "character_engine_house_lines_v0.7",
        "identity_thesis": identity_thesis,
        "houses": houses,
        "note": (
            "Person theses per house (punchy how+do); cusp modality as lived colour; "
            "no encyclopedia / no day-agenda stamp. "

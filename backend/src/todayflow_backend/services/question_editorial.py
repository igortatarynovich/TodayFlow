...
def _build_prompt(
    *,
    ...
    tarot_cards = answer_payload.get("tarot_cards")
    guidance_tarot_digest: list[dict[str, Any]] = []
    if isinstance(tarot_cards, list) and tarot_cards:
        for c in tarot_cards[:14]:
            if not isinstance(c, dict):
                continue
            guidance_tarot_digest.append(
                {
    ...
    spread_analysis = answer_payload.get("spread_analysis")
    spread_digest: dict[str, Any] | None = None
    if isinstance(spread_analysis, dict):
        spread_digest = {
            "dominant_card_name": spread_analysis.get("dominant_card_name"),
            "conflict_note": _short_text(spread_analysis.get("conflict_note"), 300),
            "themes": spread_analysis.get("themes") if isinstance(spread_analysis.get("themes"), list) else [],
        }
    ...
    if guidance_tarot_digest:
        prompt_payload["guidance_tarot_positions"] = guidance_tarot_digest
    if spread_digest:
        prompt_payload["guidance_spread_analysis"] = spread_digest

    extra = ""
    if guidance_tarot_digest:
        extra = (
            "Это ответ Guidance с раскладом: учитывай позиции карт тезисно в current_focus и next_step, "
            "не пересказывай значения полностью.\n"
        )

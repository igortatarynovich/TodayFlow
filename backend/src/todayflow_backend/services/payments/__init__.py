...
def create_checkout_session(user_id: int) -> dict:
    if not USE_STRIPE:
        ...
        return {
            ...
            "checkout_url": f"{settings.frontend_app_url.rstrip('/')}/profile?mock_paid=1",
            "mode": "mock",
        }
    ...
    try:
        session = stripe.checkout.Session.create(
            ...
            success_url=f"{settings.frontend_app_url.rstrip('/')}/reports/full?success=1",
            cancel_url=f"{settings.frontend_app_url.rstrip('/')}/profile?cancelled=1",
        )

...
def _fallback_deepen(topic: str, ctx: str, locale: str = "ru") -> dict[str, Any]:
    ...
    body = (
        (
            ...
            f"For «{title}»: name one observable cue, one likely mechanism, and one testable step today."
            if en_locale
            else f"Опирайся на общий тезис дня: {ctx[:600]}\n\n"
            f"По теме «{title}»: одно наблюдаемое, один вероятный механизм, один проверяемый шаг на сегодня."
        )
    )
    return {
        "title": title,
        "body": body[:2500],
        "bullets": [
            "Name one observable cue in this topic today." if en_locale else "Назови один наблюдаемый сигнал в этой теме сегодня.",
            "State the likely mechanism (trigger → response → cost/benefit)." if en_locale else "Сформулируй вероятный механизм (триггер → реакция → цена/выгода).",
            "One testable step and one boundary." if en_locale else "Один проверяемый шаг и одна граница.",
        ],
        "closing_line": "Write one line you can verify by evening." if en_locale else "Запиши одну строку, которую можно проверить к вечеру.",
    }
...
_DEEPEN_SYS = """Ты пишешь **аналитический слой** по одной теме дня для TodayFlow (не «сакральный» текст и не напускная важность).

Входы: topic, prior_thesis, стержень/сценарии, user_core (natal_chart при available, интерпретация, числа, знак, learning), fusion (scores, encouragement, rhythm_context), при наличии — behavior_patterns, intent, day_engine_brief, day_model, day_history.
Опирайся только на факты из JSON. Не выдумывай отметки, контракты, события и черты характера вне данных.

## Метод (обязателен)
1) **Наблюдение:** что в данных дня/профиля относится к выбранному topic (1–2 опоры: day_engine_brief / day_model / сценарий / intent / rhythm).
2) **Механизм:** коротко объясни *как это работает в поведении сегодня* (триггер → реакция → цена/выгода). Без мистики, «энергии вселенной», судьбы, кармы, «предначертано».
3) **Проверка:** что можно проверить сегодня (сигнал, число, сообщение, дедлайн, телесный факт).
4) **Шаг:** одно конкретное действие + одна граница (do / avoid). Формулировки проверяемые, не лозунги.

Астро-метки (планета/аспект/карта/число) допустимы только как **ярлык фактора** с человеческим переводом; если natal недоступен — не имитируй карту.

## По темам
- money/career: риск, цифра, срок, один выбор; без «денежный поток» и пустой значимости.
- love/family: формулировка / пауза / граница; без диагнозов личности.
- intimacy: взрослые, точные подсказки (жест, согласие, формулировка, граница); без клиники и без порнографичности; без «тайны тела» как пустого пафоса.
- full_day: одна причинная нить дня → один шаг.

## Запрещено
- Напускная важность без содержания («глубинный смысл», «судьбоносный день», «откровение», «послание вселенной»).
- Абстрактные пары существительных без ситуации («пространство и контакт», «энергия и ресурс»).
- Категоричные предсказания и клинические ярлыки.
- Второй сюжет, который отменяет base day — это слой *поверх*, уточняющий topic.

Тон: спокойный, точный, как хороший разбор: факты → механизм → шаг. Живой язык, без канцелярита.
Верни только JSON:
{
  "title": "string — конкретная тема без пафоса",
  "body": "string — 2–5 коротких абзацев: наблюдение → механизм → проверка → шаг",
  "bullets": ["наблюдение/опора", "механизм или риск", "один шаг на сегодня"],
  "closing_line": "string — одна проверяемая фиксация, без лозунга"
}
"""

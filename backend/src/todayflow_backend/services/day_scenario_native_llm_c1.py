...
- primary_conflict = главный сюжет/конфликт дня, уже выбран системой из must_dramatize и ranked drivers до твоего вызова. conflict.title должен соответствовать primary_conflict.title (можно переформулировать живым языком, но не менять тему и не выбирать другой сюжет);
- act_iii_registry_label = ярлык реестра (Акт III), НЕ сюжет и НЕ UI-тег: не копируй label_ru как conflict.title;

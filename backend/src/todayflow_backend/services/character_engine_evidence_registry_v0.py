...
def _has(facts_by_type: dict[str, dict[str, Any]], fact_type: str) -> bool:
    return fact_type in facts_by_type


_AUTONOMY_SUNS = frozenset({"aquarius", "aries", "sagittarius"})
_ANALYSIS_SUNS = frozenset({"virgo", "capricorn", "scorpio"})
_AUTONOMY_PATHS = frozenset({1, 5, 7})
_ANALYSIS_PATHS = frozenset({4, 7})
...
def _rule_autonomy(facts: dict[str, dict[str, Any]]) -> bool:
    ...
    return (sun in _AUTONOMY_SUNS) or (lp in _AUTONOMY_PATHS)


def _rule_analysis(facts: dict[str, dict[str, Any]]) -> bool:
    sun = _sign_of(facts, "planet_sign:sun")
    lp = _number_of(facts, "life_path_number")
    return (sun in _ANALYSIS_SUNS) or (lp in _ANALYSIS_PATHS)
...
def _rule_air_direction(facts: dict[str, dict[str, Any]]) -> bool:
    return element_for_sign(_sign_of(facts, "planet_sign:sun")) == "air"
...
EVIDENCE_RULES_V0: tuple[EvidenceRule, ...] = (
    EvidenceRule(
        ...
        supporting_fact_types=("planet_sign:sun", "life_path_number"),
        strengthen_fact_types=("planet_sign:mars",),
    ),
    EvidenceRule(
    ...
    EvidenceRule(
        ...
        supporting_fact_types=("planet_sign:sun", "planet_sign:moon", "life_path_number"),
        edge_type="supports",
        qualify_fact_types=("planet_sign:moon",),
    ),
    EvidenceRule(
        rule_key="air_sun_direction_v0",
        claim_kind="direction",
        thesis_key="direction_through_ideas",
        capability_floor="date_only",
        confidence="low",
        match=_rule_air_direction,

...
def runtime_is_not_wired(src_root: Path) -> bool:
    ...
    polish_modules = {
        (src_root / "services" / "today_meaning_polish_v1.py").resolve(),
        (src_root / "services" / "profile_meaning_polish_v1.py").resolve(),
    }
    attach_consumers = {
        (src_root / "services" / "day_story_wire_v1.py").resolve(),
        (src_root / "services" / "profile_contract_v1.py").resolve(),
        (src_root / "services" / "natal_decode_depth_v0.py").resolve(),
    ...
    for path in src_root.rglob("*.py"):
        ...
        if knowledge in resolved.parents or resolved == knowledge:
            continue
        if resolved == attach_gateway or resolved == consume_module or resolved in polish_modules:
            continue
        ...
        if "today_meaning_polish_v1" in text and resolved not in polish_consumers:
            return False
        if "profile_meaning_polish_v1" in text and resolved not in polish_consumers:
            return False

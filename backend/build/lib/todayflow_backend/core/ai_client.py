"""Клиент AI (OpenAI) для генерации текстов TodayFlow.

Если OPENAI_API_KEY задан — Writer может использовать LLM для прогнозов.
Без ключа используется только Lexicon (текущий поток).
"""
...
def generate_forecast_blocks(
    ...
    try:
        resp = client.chat.completions.create(
            ...
            max_tokens=1200,  # Достаточно для качественных текстов без избытка
        )

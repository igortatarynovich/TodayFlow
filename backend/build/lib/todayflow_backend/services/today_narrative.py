...
from todayflow_backend.services.generation_orchestrator import build_today_narrative_orchestration_meta
...
PROMPT_VER = "today-narrative-v16"
    ...
    *,
    ...
    for gen in q.limit(40).all():
        ...
        if str(ip.get("depth_level") or "normal") != str(depth_level or "normal"):
            continue
        if day_context_sha256:
            if str(ip.get("day_context_sha256") or "") != str(day_context_sha256):
                continue
        if prompt_label:
...
Если во входном JSON есть behavior_patterns (агрегаты событий Today за скользящее окно): это фактические сигналы взаимодействия с ритуалом и экраном (счётчики по типам событий, pattern_hints). Не выдумывай действий сверх этих данных; учитывай pattern_hints мягко в тоне и в формулировке шагов, без ярлыков и без «диагноза личности».
...
Если во входном JSON есть day_model (contract_version day_model_v0) — логический каркас дня: vector, tension, opportunity, risk, tempo, strategy и gate. headline, subline, core_message, do_items, avoid_items и action_options должны быть согласованы с vector.direction, tension.summary, risk.summary, strategy.one_focus и tempo.label; не противоречь им и не заменяй их абстрактными лозунгами.
...
1) День наружу: daily_foundation, ritual_context, fusion (scores и rhythm_context), при наличии — day_engine_brief и day_model (contract_version day_model_v0: vector, tension, risk, strategy, tempo, gate).
...
(1) Day externally: daily_foundation, ritual_context, fusion, day_engine_brief when present, and day_model (day_model_v0: vector, tension, risk, strategy, tempo, gate) when present.
...
def _system_prompt_for_surface(surface: TodaySurface, locale: str, *, depth_level: str = "normal") -> str:
    base = {
        "guide": _GUIDE_SYS,
        "day_layer": _DAY_SYS,
...
def _ru_narrative_quality_reject(texts: list[str]) -> bool:
    """True — в текстах шаблонный мусор или служебные slug'и; ответ не показываем."""
    if not texts:
        return True
...
def _guide_payload_concrete(locale: str, payload: dict[str, Any]) -> bool:
...
def _spheres_payload_concrete(locale: str, payload: dict[str, Any]) -> bool:
    if _is_en_locale(locale):
        return True
    texts: list[str] = []
...
def _dedupe_guide_payload_cross_fields(
    ...
    pool: list[str] = [t for t in (headline, subline, body_full) if t]
...
def _openai_json(system: str, user: str, *, depth_level: str = "normal") -> dict[str, Any] | None:
    ...
    *,
    ...
    if cached and isinstance(cached.normalized_response, dict):
        ...
        if surface == "guide":
            ...
            if not (
                _validate_payload_shape("guide", cached_payload)
                and _validate_payload_language("guide", cached_payload, locale_value)
                and _guide_payload_concrete(locale_value, cached_payload)
            ):
                cached = None
        if cached is not None:
            ...
            return cached_payload, cached.id, bool(cached.used_fallback), profile_selector_slim
    ...
    system_prompt = _system_prompt_for_surface(surface, locale_value, depth_level=depth_norm)
    pv = learning.get_or_create_prompt_version(
        ...
        metadata={"surface": surface, "locale": locale_value, "depth_level": depth_norm},
    )
    ...
    try:
        ...
        if surface == "guide":
            ...
            if is_llm_chat_configured():
                s1_txt, s2_txt = funnel_system_prompts_for_locale(locale_value)
                ...
                sat_part, interp_part, funnel_meta = run_guide_narrative_funnel_v0(
                    funnel_openai_json_adapter,
                    locale_value=locale_value,
                    tier_norm=tier_norm,
                ...
                if isinstance(interp_part, dict):
                ...
                if isinstance(sat_part, dict) and not funnel_meta.get("failed"):
                    ...
                    payload_try = _ensure_guide_actionable_fields(
                        payload_try, foundation, fusion_for_prompt, locale_value
                    )
                    if (
                        _validate_payload_shape("guide", payload_try)
                        and _validate_payload_language("guide", payload_try, locale_value)
                        and _guide_payload_concrete(locale_value, payload_try)
                    ):
                        merged_try = _merge_guide_why_astrological_layers(
                            payload_try, foundation, fusion_for_prompt, core_profile, locale_value
                        )
                        if (
                            _validate_payload_language("guide", merged_try, locale_value)
                            and _guide_payload_concrete(locale_value, merged_try)
                        ):
            ...
            if not funnel_ok:
                payload = _openai_json(system_prompt, user_prompt, depth_level=depth_norm)
                used_fb = True
                if payload and payload.get("headline"):
                    ...
                    if _validate_payload_shape("guide", payload) and _validate_payload_language(
                        "guide", payload, locale_value
                    ):
                        merged_guide = _merge_guide_why_astrological_layers(
                            payload, foundation, fusion_for_prompt, core_profile, locale_value
                        )
                        if (
                            _validate_payload_language("guide", merged_guide, locale_value)
                            and _guide_payload_concrete(locale_value, merged_guide)
                        ):
            ...
            if (
                ...
                and _validate_payload_shape("day_layer", payload)
                and _validate_payload_language("day_layer", payload, locale_value)
            ):
                used_fb = False
            else:
                ...
                payload = _fallback_day_layer(enc, [str(x) for x in recs], locale_value)
            ...
            if (
                ...
                and _validate_payload_shape("spheres", payload)
                and _validate_payload_language("spheres", payload, locale_value)
                and _spheres_payload_concrete(locale_value, payload)
            ):
                used_fb = False
            else:
            ...
            if (
                ...
                and _validate_payload_shape("evening", payload)
                and _validate_payload_language("evening", payload, locale_value)
            ):
                used_fb = False
            else:
            ...
            if (
                ...
                and _validate_payload_shape("deepen", payload)
                and _validate_payload_language("deepen", payload, locale_value)
            ):
                used_fb = False
            else:
    ...
    if surface == "guide" and isinstance(payload, dict):
        ...
        payload = _ensure_guide_actionable_fields(payload, foundation, fusion_post, locale_value)
        payload = _merge_guide_why_astrological_layers(payload, foundation, fusion_post, core_profile, locale_value)
        if ritual_norm:
            payload = _append_ritual_why_layers(payload, ritual_norm, locale_value)
        _gd_p = (day_ctx.get("layers") or {}).get("guide_decision")
        payload = _apply_guide_decision_to_guide_payload(
            payload, _gd_p if isinstance(_gd_p, dict) else None, locale_value
        )
        payload = _ensure_guide_actionable_fields(payload, foundation, fusion_post, locale_value)
        payload = _dedupe_guide_payload_cross_fields(payload, foundation, fusion_post, locale_value)
        payload = _normalize_guide_payload_for_tier(payload, tier_norm)
        payload.pop("funnel_contract", None)
    ...
    gen = learning.log_generation(
        ...
        model=settings.llm_default_model if is_llm_chat_configured() else None,
        locale=locale_value,

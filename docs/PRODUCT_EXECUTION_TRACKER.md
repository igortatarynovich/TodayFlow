...
- 2026-07-22 | Product | **Data Providers: LLM facts (DeepSeek) · no astro microservice** | **ACTIVE / DRAFT** | SoT: [PRODUCT_DATA_PROVIDERS.md](./PRODUCT_DATA_PROVIDERS.md) v0.3 — карта (планеты/ASC/дома) = structured LLM JSON; без отдельного астро-сервиса; geo ETL deferred. Next: `NatalChartFacts` schema + fact prompt passport.
...
- 2026-07-22 | Product | **Availability Matrix + Profile/Today freeze** | **ACTIVE / DRAFT** | SoT: [PRODUCT_AVAILABILITY_MATRIX.md](./PRODUCT_AVAILABILITY_MATRIX.md) — слой 0 intake · слой 1 birth data · слой 2 экраны×доступ · слой 3 блоки. **До APPROVED:** не менять Profile/Today IA/LLM. Next: закрыть Open decisions (Trial, Free base cut, Guest Profile demo, Plus vs Pro).

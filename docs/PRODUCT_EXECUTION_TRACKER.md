...
- 2026-07-29 | Today / Content | **Wave 2 Phase A — TapWidget** | **IN PROGRESS / code** | `POST /today/tap-widget/response` + `GET /today/accuracy-summary` + FE TapWidget (prompt from scene.trap, motion pilot). Architecture: tap_event_v1 SoT for accuracy loop; scenes from day_scenario until full day_facts_v1. Dictionary 0.5.2a approved; 0.5.2b consecutive August still open.
- 2026-07-29 | Today / Content | **Wave 2 Phase 0.5 pass 2** | **DICT APPROVED · 0.5.2b OPEN** | Dictionary `calm|charged|friction|open` approved. 8-date calib window = one Leo–Aquarius career season (not formula bug); year-spread 12 dates differentiates. Still open: 10–14 consecutive days in one charged month for day-to-day detail. Phase A code starting. [CONTRACT §3.1b](./today/TODAY_WAVE2_CONTRACT_V1.md).
...
- 2026-07-29 | Profile / UI | **Profile motion accents (B)** | **IN BRANCH** | Decode CTA attention-breathe while unopened · one-shot pattern sweep + aspect wave on first grounded Decode · Act2 `selected_by` one-shot reveal · Sun/Moon/ASC/MC tap expand+shadow · `profileMotionOnce` localStorage · Act1 idle · 3D still go/no-go after live QA

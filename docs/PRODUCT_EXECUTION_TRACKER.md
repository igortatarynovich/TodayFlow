...
**NOW (CLOSED LOOP v0, 2026-09-18):** Production evening is gratitude only (no leftover «Закрыть день» on ScreenFlow). Yesterday close persists on `POST /day-connection/{date}` (`evening_completed` + `evening_observations.kind=gratitude`); D+1 TODAY reads `GET` yesterday (`T1.continuity`) with same-device local fallback. MY DAY practice uses `GET /practices/select` from Global `primary_energy` → need cell; unmatched omits (no `/practices?limit=1` invent). PR https://github.com/igortatarynovich/TodayFlow/pull/21 — `ci.yml` now `grep -E` + `cancel-in-progress` (was blocked on missing `workflow` scope). Merge after iOS Smoke green on that SHA. Run 3 gratitude (live): calendar D+1 OK; UI claim «Мария» OK (SMTP unset → JWT fallback); Playwright evening 1/1. Leftover: wall-clock evening ≥18:00 MSK; two calendar days same person. Handoff: `docs/status/CLOSED_LOOP_V0_HANDOFF.md`. G0 deferred.

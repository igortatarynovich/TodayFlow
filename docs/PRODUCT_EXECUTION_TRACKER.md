...
- 2026-07-30 | Today / Content | **Wave 2 Phase D.1b — narrative on day_facts** | **LIVE** | Project cached `day_scenario` onto `GET /today/day-facts` when `conflict.driver_ids ⊆` fresh activations; else `partial: true` (no invent). thesis=`label_ru`|null; evening_payoff null. Act3 still day_scenario nest; not-ready → Day Map/legacy. [CONTRACT](./today/TODAY_WAVE2_CONTRACT_V1.md) · [PLAN D.1b](./today/TODAY_WAVE2_EXECUTION_PLAN.md).

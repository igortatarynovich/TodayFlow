...
- 2026-07-25 | Today / Architecture | **day_thesis + evidence pack in DayContext** | **IN_PROGRESS** | Arch correction: `day_events_pack_v1` nested under `DayContext.layers.evidence` (not parallel SoT) · `day_thesis_v1` (family/variant/mode) replaces machine `primary_conflict` · compositions in ranker · coherence gate only (not TL-1) · language canon path: `docs/today-language/` · **PR order:** evidence→ranker/thesis→story schema→FE parity→language calibration · linked: [DAY_ENGINE_AND_COHERENCE.md](./DAY_ENGINE_AND_COHERENCE.md), [DATA_OWNERSHIP_AND_CONSUMPTION_MAP.md](./DATA_OWNERSHIP_AND_CONSUMPTION_MAP.md)

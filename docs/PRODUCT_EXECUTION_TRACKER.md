...
**NOW (P1 PRACTICE LIBRARY FILL, 2026-08-31):** P0 fill complete: all 26 P0 need cells sourced, 40/44 P0 spine types accepted (`box_breathing`, `energizing_breath`, `abstinence`, `self_trust` skipped). P1 coverage started: `practice.paced_breathing` sourced via `technique.paced_breathing` (NHS Bradford District Care Trust Ready to Relate + Harvard Health) — slow, steady breath (~5–6 breaths/min), inhale through the nose, exhale slightly longer, no hold. `practice.paced_breathing.001` is active with `technique_id`; coverage matrix now 134 library items, 42 P1 types, 1 sourced. Content Library hub serves 112 active/accepted content-library items + legacy catalog. Tests green: `test_content_library_v1.py` (52), `test_content_library_selection_v1.py` (13), `test_practices.py` (27). Mapping updated: `paced_breathing` → category `breathing`, format `breath`. Next: `practice.physiological_sigh` (next P1 type in ledger order).

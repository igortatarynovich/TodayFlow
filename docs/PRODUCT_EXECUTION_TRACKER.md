...
**NOW (PRACTICES / CANON, 2026-08-30):** **Library fill** — next cell `need.discipline.prepare`. Sourced 18/26. Latest: `need.recovery.recover` via `progressive_relaxation` (fists on inhale, open on exhale, three times; NHS inform progressive muscle relaxation + Mayo Clinic relaxation techniques + VA Whole Health Library PMR). `box_breathing`, `energizing_breath`, and `self_trust` skipped. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md). Meaning still does not emit `item_id` or `technique_id`.
...
- **SoT after:** fill = lightweight provenance (source check → own-words description → safety notes if material → accepted/skipped → Content Item). Research ladder = archive, non-blocking. `box_breathing` = `skipped_for_now`. Next Product = library fill at `need.discipline.prepare`. 133 items remain `llm_provisional`. Taxonomy classes/types, coverage cells, item contract field groups, and Meaning boundary unchanged.
- **Public contract changed?** no

...
**NOW (PRACTICE LIBRARY FILL, 2026-08-29):** P0 cells `need.emotional_awareness.reflect` (via `technique.self_check_in`) and `need.self_connection.reflect` (via `technique.journaling`) are sourced. Sources: Greater Good Science Center Naming Your Emotions + NHS Lothian Emotion Workbook + Torre & Lieberman (2018) affect labeling + Nook et al. (2022) timing/intensity caution for `self_check_in`; NHS Lanarkshire Writing for Wellbeing + CUH NHS Write Your Self + Greater Good Science Center Expressive Writing for `journaling`. `practice.self_check_in.*` and `practice.journaling.*` items are `active` with `technique_id`. Coverage matrix: 13/26 covered, 0 empty, 13 seed; next cell `need.connection.connect`. Technique canon: 13 accepted, 3 skipped (`box_breathing`, `energizing_breath`, `self_trust`). Tests green: `test_content_library_v1.py` (51). Canon docs updated: `PRACTICE_LIBRARY_FILL_V1`, `PRACTICE_TECHNIQUE_PROVENANCE_V1`, `PRACTICE_CONTENT_COVERAGE_V1`, `PRACTICE_CONTENT_TAXONOMY_V1`. Next: source `need.connection.connect` (connection_action), or another non-LLM foundation task.
...
**NOW (PRACTICES / CANON, 2026-08-29):** **Library fill** — next cell `need.connection.connect`. Sourced 13/26. Latest: `need.self_connection.reflect` via `journaling` (brief three-sentence unedited private writing; NHS Lanarkshire Writing for Wellbeing + CUH NHS Write Your Self + Greater Good Science Center Expressive Writing). `box_breathing`, `energizing_breath`, and `self_trust` skipped. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md). Meaning still does not emit `item_id` or `technique_id`.
...
- **SoT after:** fill = lightweight provenance (source check → own-words description → safety notes if material → accepted/skipped → Content Item). Research ladder = archive, non-blocking. `box_breathing` = `skipped_for_now`. Next Product = library fill at `need.connection.connect`. 133 items remain `llm_provisional`. Taxonomy classes/types, coverage cells, item contract field groups, and Meaning boundary unchanged.
- **Public contract changed?** no

...
**NOW (PRACTICES / CANON, 2026-08-30):** **Library fill** — next cell `need.presence.stabilize`. Sourced 24/26. Latest: `need.reset.release` via `digital_pause` (move device out of reach or face down and do not interact for a short bounded period; NHS Every Mind Matters screen breaks + NHS Employers DSE guidance). `box_breathing`, `energizing_breath`, `abstinence`, and `self_trust` skipped. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md). Meaning still does not emit `item_id` or `technique_id`.

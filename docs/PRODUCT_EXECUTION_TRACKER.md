...
**NOW (PRACTICES / CANON, 2026-08-30):** **P0 type coverage** — all 26 P0 need cells sourced; now filling P0 spine types. Latest: `practice.free_writing` accepted (Peter Elbow 1973 + MIT Writing Process): write continuously for a set time without stopping, editing, or erasing. `practice.free_writing.001/002/003` active with `technique_id`. P0 spine type coverage: 28/44 sourced. `box_breathing`, `energizing_breath`, `abstinence`, and `self_trust` remain skipped. Next non-skipped P0 spine type: `practice.morning_ritual`. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md). Meaning still does not emit `item_id` or `technique_id`.

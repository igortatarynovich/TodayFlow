...
**NOW (PRACTICE LIBRARY FILL, 2026-08-29):** P0 cells `need.emotional_awareness.reflect` (via `technique.self_check_in`), `need.self_connection.reflect` (via `technique.journaling`), and `need.connection.connect` (via `technique.connection_action`) are sourced. Sources: Greater Good Science Center Naming Your Emotions + NHS Lothian Emotion Workbook + Torre & Lieberman (2018) affect labeling + Nook et al. (2022) timing/intensity caution for `self_check_in`; NHS Lanarkshire Writing for Wellbeing + CUH NHS Write Your Self + Greater Good Science Center Expressive Writing for `journaling`; NHS Essex ICB Looking after your mental health + Liu et al. (2025) systematic review on behavioral activation for social connection + Laidlaw et al. (2020) tele-delivered behavioral activation for connectedness for `connection_action`. `practice.self_check_in.*`, `practice.journaling.*`, and `practice.connection_action.*` items are `active` with `technique_id`. Coverage matrix: 14/26 covered, 0 empty, 12 seed; next cell `need.creativity.open`. Technique canon: 14 accepted, 3 skipped (`box_breathing`, `energizing_breath`, `self_trust`). Tests green: `test_content_library_v1.py` (51). Canon docs updated: `PRACTICE_LIBRARY_FILL_V1`, `PRACTICE_TECHNIQUE_PROVENANCE_V1`, `PRACTICE_CONTENT_COVERAGE_V1`, `PRACTICE_CONTENT_TAXONOMY_V1`. Next: source `need.creativity.open` (creative_prompt), or another non-LLM foundation task.

...
**NOW (PRACTICES / CANON, 2026-08-30):** **P0 type coverage** — all 26 P0 need cells sourced; now filling P0 spine types. Latest: `practice.environment_reset` accepted (Mind.org.uk + Leicestershire Partnership NHS Trust): reset immediate physical environment by removing/relocating/tidying one object or clearing a surface before next task. `practice.environment_reset.001/002/003` active with `technique_id`. P0 spine type coverage: 27/44 sourced. `box_breathing`, `energizing_breath`, `abstinence`, and `self_trust` remain skipped. Next non-skipped P0 spine type: `practice.free_writing`. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md). Meaning still does not emit `item_id` or `technique_id`.

...
**NOW (X11 CLOSED / PASS, 2026-09-21):** One Inventory narrative on locked production Today. Product `/today` always mounts the 4-surface; `?full=1` / `?experience=1` are no-ops. Glance-as-act not constructed. Evening is gratitude, not promise-trap. Leftover files stay. TIC stays **CLOSED / PASS**. X3 stays **CLOSED / PASS**. X4/X5 stays **CLOSED / PASS**. N = 20. Next remainder = **X14**. Not TIC-K21. Not landing. Branch `cursor/x11-leftover-narrative`. Do not rebuild server. P1 fill STOPPED. G0 deferred. Closed loop LOCKED.

**NOW (X11 GATE SELECTION, 2026-09-21):** Next executable Today gate = Full User Path **X11 leftover narrative**. One PASS. Production locked Today must not speak a parallel leftover story. Named leftovers: Glance-as-act · `?experience=1` / `?full=1` · promise-trap / duplicate `day_story` · deferred `todayRitualCopy`. **X14 after X11.** Not two trains. Not TIC-K21. Not landing. Selection only — no product paint. Stay on `cursor/x4-x5-honest-reveal-copy` until execution starts (then new branch). TIC stays **CLOSED / PASS**. X3 stays **CLOSED / PASS**. X4/X5 stays **CLOSED / PASS**. N = 20. Do not rebuild server. P1 fill STOPPED. G0 deferred. Closed loop LOCKED.
...
**NOW (TIC-K13 COMPLETE, 2026-09-21):** `T2.lens_card` / `card.hook_reveal.personal_angle` is Personal Day × F13 after persist. Global chorus `day_card` / `bridge_to_day` stay on the Global hook and do not feed K13. Kitchen / PIC / CE / chrome / copied K06–K10 do not feed the slot. Missing personal lens omit. Glance leftover is not this hop. TIC_COVERAGE K13 PARTIAL → COMPLETE (**15 COMPLETE / 5 PARTIAL / 0 MISSING / 0 OMIT-BY-DESIGN**). Next remaining = **K14** only. Not IL. Not PIC. Branch `cursor/today-information-contract`. Do not rebuild server. P1 fill STOPPED. G0 deferred. Closed loop LOCKED.
...
**NOW (PIC K03 COMPLETE, 2026-09-20):** Applied ASC/MC and occupied-house how/do come from F05/F06 IL-2 (angle orientation × sign manner; planet×house arena) into Explore `P6.applied.*`. Full natal only. Not a 12-house encyclopedia, not K07 spheres, not Decode/tips fallback, not P1–P5. Missing occupancy/atom omits. PIC_COVERAGE K03 PARTIAL → COMPLETE (16 COMPLETE / 1 PARTIAL / 0 MISSING / 1 OMIT-BY-DESIGN). Remaining: **K01 thesis mint** as a separate decision — do not start Today until that close or conscious defer. Not more IL. Branch `cursor/profile-knowledge-to-output`. Do not rebuild server. P1 fill STOPPED. G0 deferred. Closed loop LOCKED.
...
**NOW (PIC K04 COMPLETE, 2026-09-20):** `P3.help` is one Internal Engine axis from Stage 0 F08 `element_balance` (F03/F04 tilt) or one harmonic F07; not seven widgets. Identity-thesis engine cannot occupy the path axis; missing tilt/harmonic omits. K05 keeps `P3.insight`. PIC_COVERAGE K04 PARTIAL → COMPLETE (10 COMPLETE / 7 PARTIAL / 1 MISSING). Next = **K09** only. Not Today. Not more IL. Branch `cursor/profile-knowledge-to-output`. Do not rebuild server. P1 fill STOPPED. G0 deferred. Closed loop LOCKED.
...
**NOW (P1 SELF IDENTITY SKIP, 2026-09-20):** P1 type `affirmation.self_identity` skipped (`source_gap`). No brief spoken «I can remain myself» method; Steele/Cohen is values-writing; CCI/NHS self-esteem is worth/esteem; slogan is not a method. Coverage 153 items, 20/42 P1 sourced, 3 P1 skipped. Next: `affirmation.self_worth`. Handoff: `docs/status/P1_LIBRARY_FILL_HANDOFF.md`. G0 deferred. Closed loop LOCKED.
...
**NOW (P1 SELF COMPASSION, 2026-09-19):** P1 type `meditation.self_compassion` sourced via `technique.self_compassion` (KMPCT NHS stress handbook + SE Trust HSCNI Using Self Compassion + CCI WA Health Barriers to Self-Compassion): sit still, notice a hard moment or harsh self-talk, answer as you would a friend, stop. Distinct from loving_kindness, acceptance, affirmation.compassion, and journaling. Not full MSC curriculum, CFT, or treatment. `meditation.self_compassion.001` active. Coverage 151 items, 18/42 P1 sourced. Next: `meditation.gratitude`.

**NOW (P1 LOVING KINDNESS, 2026-09-19):** P1 type `meditation.loving_kindness` sourced via `technique.loving_kindness` (NHS Scotland Right Decisions actual practice + NHS Lothian Loving Kindness meditation + BSMHFT sending kind thoughts): sit still, wish yourself well, then one person you care about, stop. Distinct from self_compassion, connection_action, and meditation.gratitude. Not full MBSR four-circle protocol, CFT, or treatment. `meditation.loving_kindness.001` active. Coverage 150 items, 17/42 P1 sourced. Next: `meditation.self_compassion`.
...
**NOW (P1 WALKING, 2026-08-31):** P1 type `practice.walking` sourced via `technique.walking` (NHS sit less + Better Health get active + NHS England corridor/2-minute walk): stand, walk a short comfortable distance, stop. Distinct from mobility, mindful_movement, and walking_meditation. `practice.walking.001` active. Coverage 139 items, 6/42 P1 sourced. Next: `practice.shaking_release`.

**NOW (GLANCE AUTHORITY, 2026-08-31):** Glance Daily Focus is not a fifth Today act and not an importance layer. Production `T3.priority` = Personal `do[]`, else persisted `today_move` (not Global expect/chips, not `buildGlanceDailyFocus`). Synthesizer stays on legacy `?experience=1` only. `TodayGlanceAct` remains unmounted. Dead `ProfileCharacterScene.tsx` deleted. Tests: `todayMyDayPriority` + live frames. Leftover Glance files stay post-MVP like other launch-cut surfaces.

**NOW (P1 MINDFUL MOVEMENT, 2026-08-31):** P1 type `practice.mindful_movement` sourced via `technique.mindful_movement` (Sussex Mindfulness Centre NHS + UHDB NHS + RUH Bath PHY051): sit or stand, one slow comfortable movement, attention on the moving body. Distinct from mobility, stretching, sitting mindfulness, and walking. `practice.mindful_movement.001` active. Coverage 138 items, 5/42 P1 sourced. G1 leftover on `main`: iOS XCTest Smoke fails because CI still calls `rg` (not on macOS runners); local `ci.yml` now uses `grep` — push needs GitHub `workflow` scope. Next: `practice.walking`, or land the iOS smoke workflow fix.

**NOW (PROD DEPLOY, 2026-08-31):** Live compose rebuilt on this host: `docker compose -f docker-compose.prod.yml up -d --build --force-recreate backend frontend`. Base SHA `1166ab96` (PR #18 / G1 without workflow) plus leftover product files now committed to `main` (practice `stretching` + skip `practice.body_scan`, Grammar §9 emit helpers, Today/Profile display leftovers). Postgres crashed mid-cutover (`No space left on device`); Docker build cache/dangling images pruned (−38 GB, disk 100% → 26% / 54G free); postgres restarted, `database system is ready`. Server check: `/health` 200 production; landing/today/profile/compatibility/tarot 200; `/practices` 162; `/tarot/cards/21` 200. LLM latch still tripped (`spent_usd≈5.005`, `tripped=true`) — G0 still deferred. Next: G1 leftovers or owner call on billing.

**NOW (PHASE 1.1–1.2 CLOSED, 72 BACKEND FAILURES LEFT, 2026-08-31):** Phase 1.1 account delete cascade fixed: `CachedNatalChart.astro_profile` now cascades `all, delete-orphan`, `test_account.py` delete test passes (23/26 passed, 3 remaining core-v3 vs core-v2 baseline failures). Phase 1.2 day-engine knowledge personalization cluster closed: the root cause was time-dependent tests — fixture `last_confirmed_at=2026-05-31` became stale by 2026-08-31, causing `select_knowledge_context_v1` to drop facts below `MIN_SELECTION_SCORE=0.25`. Fixed by injecting `now` through `build_day_context_v0` → `wire_active_knowledge_into_day_context_layers` → `select_knowledge_context_v1` and passing a fixed `NOW=2026-05-31` in the affected test modules (5 files, 52 tests). Full backend suite now **72 failed / 2487 passed**. Original MONEY_ACTION_FALLBACK / Today contract cluster and Compatibility guest blocks smoke were already green at HEAD (fixed by weekend commits). Next: triage the remaining 72 failures into launch-critical vs stale/legacy/LLM-infra and pick the next cluster.

**NOW (LAUNCH BLOCKERS CLOSED, 2026-08-31):** UI-walkthrough на локальном prod-like стеке (`docker-compose.walkthrough.yml`, изолированные порты/ledger) пройден: Landing → Onboarding → First Today → Profile → Today 4 steps → Compatibility → Tarot, Evening отдельно, D+1 continuity. Найдены и закрыты три launch-блокера. **#1 (fallback semantics, Architecture impact):** при billing-suspended Nebius auth Today висел на бесконечной шторке — `day_story_is_product_ready` отвергал `facts_only_no_llm`. Теперь GET miss по `/today/contract` строит и отдаёт детерминированный пакет inline (`allow_rebuild_on_miss=True`, `force_rebuild=False` ⇒ 0 LLM по построению); cron assemble-window остаётся владельцем native LLM upgrade (`facts_only` не product-ready ⇒ пересобирается на месте при возврате LLM). FE: шторка+polling только для truly-empty shell (`isDayAssembling && !day_story`). Канон обновлён: `COMPUTE_LIFECYCLE_AND_ARTIFACT_ECONOMICS_V1.md` (GET miss → deterministic first-build). **#2:** guest first-today reload выбрасывал на pitch — completion теперь day-keyed (`isFirstTodayCompleteForOtherDay`, legacy-записи без `day_key` = предыдущий день). **#3:** петля save↔refine при пропуске города — `CoreSetupPayload.location_name` Optional (public contract: backward compatible), `claimGuestProfileAfterAuth({allowMissingLocation})` на refine-skip пути, профиль создаётся на «general» глубине. Evening persistence и D+1 «помнит вчера» подтверждены через API и e2e (`evening-d1-continuity.spec.ts`: динамические даты Europe/Moscow, port-scoped мокинг — Accept header отсутствует в FE getJson, старые glob-моки текли в реальный backend). Верификация на стеке: #1/#2/#3 PASS. Тесты: jest `firstTodayState` 8/8; `test_account.py` 22 passed / 4 failed (baseline `core-v3` vs `core-v2` кластер, воспроизводится на HEAD); tsc чисто. Коммиты: `5fad078a` (deterministic day on GET miss), `2cca1d2e` (claim без location), `a368b4b0` (e2e robustness) + ранее `2878b0cd` (day-key first-today). Next: CI green на `a368b4b0` → решение владельца о деплоии на live + smoke `todayflow.today`.
...
**NOW (P1 PRACTICE LIBRARY FILL, 2026-08-31):** P0 fill complete: all 26 P0 need cells sourced, 40/44 P0 spine types accepted (`box_breathing`, `energizing_breath`, `abstinence`, `self_trust` skipped). P1 coverage: 3/42 sourced — `practice.paced_breathing`, `practice.physiological_sigh`, and `practice.grounding` via `technique.practice_grounding` (Royal Berkshire NHS + Singh et al. 2003 Soles of the Feet): sit or stand, press both feet firmly into the floor, notice sole contact. Distinct from `sensory_grounding` and `meditation.grounding`. Coverage matrix now 136 library items. Content Library hub serves 114 active/accepted content-library items + legacy catalog. Tests green: `test_content_library_v1.py` (54), `test_content_library_selection_v1.py` (13), `test_practices.py` (27). Next: `practice.body_scan` (next P1 type in ledger order).
...
- **SoT before:** X14 selected; completeness / placement named as remainder, not measured.
- **SoT after:** X14 OPEN on execution branch. Audit: `T1.continuity` PARTIAL · `T3.tracker` PARTIAL. No paint. PASS still both slots complete and placed on the locked 4-surface. Next hop = minimal implementation against those two rows.
- **Public contract changed?** no
- **Migration required?** no
- **Canon updated?** yes — this tracker · TODAY_PRODUCT_FLOW · FULL_USER_PATH §0/§13/§16 · X14 handoff
- **Backward compatible?** yes. Audit only; no product paint.

## Architecture impact — X14 gate selection (2026-09-21)

- **SoT before:** X11 CLOSED / PASS; next remainder named «X14» without hop shape. §13 one OPEN row (progress); §16.1 п.8 already one P1 item (progress + D2).
- **SoT after:** next executable Today gate = **X14** only. PASS = locked `T1.continuity` (D2 gratitude recall) and `T3.tracker` (habit micro-progress) are complete and placed on the locked 4-surface. Not two sequential product trains. Selection only; no paint in this hop.
- **Public contract changed?** no
- **Migration required?** no
- **Canon updated?** yes — TODAY_PRODUCT_FLOW selection · FULL_USER_PATH §0/§13/§16 · this tracker · X11 handoff pointer
- **Backward compatible?** yes. Selection only; no product paint.

## Architecture impact — X11 leftover narrative (2026-09-21)

- **SoT before:** X11 OPEN after selection. Production already no-op'd leftover query params via `NODE_ENV`; same `/today` route still imported `TodayExperienceSurface` / `TodayRitualFlow` for development. Dead `glanceSection` still constructed on the locked surface.
- **SoT after:** **X11 CLOSED / PASS.** Product `/today` always mounts the locked 4-surface. Query params cannot switch leftover surfaces. Glance leftover is not constructed on that surface. Evening stays gratitude. Leftover files remain; physical delete out of gate. **Next remainder = X14.**
- **Public contract changed?** no
- **Migration required?** no
- **Canon updated?** yes — TODAY_PRODUCT_FLOW · FULL_USER_PATH §0/§13/§16 · this tracker · X11 handoff
- **Backward compatible?** yes. `?full=1` / `?experience=1` already no-op in production; now no-op in the route itself.

## Architecture impact — X11 gate selection (2026-09-21)

- **SoT before:** X4/X5 CLOSED / PASS; next named «X11 then X14» without hop shape.
- **SoT after:** next executable Today gate = **X11** only. X14 stays after. Selection only; no leftover paint in this hop.
- **Public contract changed?** no
- **Migration required?** no
- **Canon updated?** yes — TODAY_PRODUCT_FLOW selection · FULL_USER_PATH §0/§13/§16 · this tracker · X4/X5 handoff pointer
- **Backward compatible?** yes. Selection only; no product paint.
...
- **SoT before:** X3 CLOSED / PASS; next named «X4/X5» without pinning one hop vs two. §13 lists two IDs; §16.1 п.4 already one P0 item.
- **SoT after:** next executable Today gate = **one joint X4/X5**. PASS only when both clauses hold on locked T2-gate chrome. Order inside the hop = card then number (existing A→B). Not two sequential product trains.
- **Public contract changed?** no
- **Migration required?** no
- **Canon updated?** yes — TODAY_PRODUCT_FLOW selection · FULL_USER_PATH §0/§13/§16 · this tracker · X3 handoff pointer
...
- **SoT before:** TIC CLOSED / PASS; next gate unnamed «from canon». FULL_USER_PATH §13 still listed X3 as ritual-first in code.
- **SoT after:** next Today product gate = **X3** Theme/Focus/Step experience on closed N. Ritual-first funnel is already closed. X4–X5 then X11/X14. Compatibility Information Contract is not next.
- **Public contract changed?** no
- **Migration required?** no
- **Canon updated?** yes — TODAY_PRODUCT_FLOW selection · FULL_USER_PATH §0/§13/§16 · Information Contract §12 · this tracker · frozen handoff
- **Backward compatible?** yes. No product paint in this hop.
...
- **SoT before:** Scene `props.affirmations[0]` / trap / `recommended_action` fed `T3.affirmation`. Date-hash rotation picked practice vs affirmation when both existed. Catalog miss could fill with scene affirmation.
- **SoT after:** K17 is XOR from the existing K16 F10 need-cell class. Exactly one slot. Scene affirmation is not Personal verbal support. Empty selected branch omits. No new selector/ranker. K16 select unchanged. Not K20 extraCards. Glance leftover is not this hop.
- **Public contract changed?** no JSON fields. `practice_recommendation` may be empty on an ok interpretation.
- **Migration required?** no. Cached scene-affirmation rec omits on the locked surface.
- **Canon updated?** yes — Information Contract §3/§10/§11 · Display Inventory `T3.affirmation` · this tracker · handoff
- **Backward compatible?** yes for API. MY DAY never shows both; affirmation omits without a grounded branch.
...
- **SoT before:** Global chorus `day_number` packaged as `bridge_to_day` and painted as `T2.lens_number`. Persist / deep-depth gate existed; provenance was chorus.
- **SoT after:** `T2.lens_number` = Personal Day × F11/F12 after persist (`personal_angle`). I0-locked Global chorus stays on `interpretive_chorus.day_number` / `bridge_to_day` and does not feed K14. Missing personal lens omit. Not a second ranker. Not a copy of K06–K10 or K13. Not K15+.
- **Public contract changed?** no new JSON fields. `personal_angle` remains omit without Personal×number.
- **Migration required?** no. Cached chorus-as-lens omits on FE until a Personal×number line exists.
- **Canon updated?** yes — Information Contract §3/§11 · Display Inventory `T2.lens_number` · this tracker · handoff
- **Backward compatible?** yes for API. Ritual number «Для тебя сегодня» omits without Personal×number.

## Architecture impact — TIC-K13 card lens (2026-09-21)

- **SoT before:** Global chorus `day_card` packaged as `bridge_to_day` and painted as `T2.lens_card`. Persist gate existed; provenance was chorus.
- **SoT after:** `T2.lens_card` = Personal Day × F13 after persist (`personal_angle`). I0-locked Global chorus stays on `interpretive_chorus.day_card` / `bridge_to_day` and does not feed K13. Missing personal lens omit. Not a second ranker. Not a copy of K06–K10. Not K14+.
- **Public contract changed?** no new JSON fields. `personal_angle` remains omit without Personal×card.
- **Migration required?** no. Cached chorus-as-lens omits on FE until a Personal×card line exists.
- **Canon updated?** yes — Information Contract §3/§11 · Display Inventory `T2.lens_card` · this tracker · handoff
- **Backward compatible?** yes for API. Ritual «Для тебя сегодня» omits without Personal×card.
...
- **SoT before:** Global scene `recommended_action` / goals packaged as personal `do[]`; FE painted `T3.priority`.
- **SoT after:** `do[]` = Personal Narrative after bind only. I0-locked Global action is not the slot. Missing personal do omit. Not a second ranker. Not K10.
- **Public contract changed?** yes — `do[]` / `today_move` may be empty on ok interpretation (omit). No new JSON fields.
- **Migration required?** no. Cached Global-as-do omits on FE until rebuild.
- **Canon updated?** yes — Information Contract §3/§11 · Display Inventory `T3.priority` · this tracker · handoff
- **Backward compatible?** yes for API. Priority omits without Personal Narrative do.
...
- **SoT before:** Stage 2 minted K01 from the 13-key Sun/Moon/ASC registry (`_DETERMINISTIC_SURFACE_BY_IDENTITY` / LLM). Occupancy IL-2 lemmas were fill-empty qualifiers on that canned phrase. Leo and other suns outside the 13 buckets could not ground.
- **SoT after:** K01 is one observable mechanism composed from allowed facts: sun `core_function` × sign `manner` (mechanism), moon manner (hold), full-natal ASC orientation × manner (doorway), F09 keyword (contribution), occupied non-luminary IL-2 as qualifier. `compose_k01_identity_v0`. Each piece traces to a claim/fact. Missing evidence drops the piece — no generic personality essay. 13-key registry is fallback only if sun IL-2 cannot compose. Occupancy concretizes; it does not replace the mechanism. LLM cannot overwrite a composed surface.
- **Public contract changed?** no new required JSON fields. Identity Core may carry `recognition_line`, `k01_source`, `k01_pieces`. Thesis key for a composed sun is `planet_in_sign:sun:{sign}` rather than `builds_through_*`.
- **Migration required?** no — clients already read `recognition_line` / `identity_core` strings. Cached cores keep old 13-key prose until rebuild.
- **Canon updated?** yes — Information Contract §3/§11 · Display Inventory `P1.recognition_line` / `P1.identity_core` · this tracker · handoff
- **Backward compatible?** yes for P2–P6. P1 logline wording changes for every chart that can compose. Decode still treats Identity Core as fixed input.

## Architecture impact — PIC-K03 applied angles/houses (2026-09-20)

- **SoT before:** `character_engine_house_lines_v0` emitted all 12 houses from identity-thesis / cusp-sign banks with empty `do`; ASC/MC used sign-modality templates; PIC mapped K03 onto P2 anchors; IL-2 arena/orientation was not the source.
- **SoT after:** K03 is Explore/Map applied how/do. Occupied Sun–Saturn houses = IL-2 `compose_planet_in_house`. ASC/MC = F05 angle `orientation` × sign `manner`. Full natal only. Unoccupied houses omit. Not a K07 sphere, not a new K01 thesis, not Decode/tips fallback. DSC/IC stay out of V1. Inventory `P6.applied.asc/mc/house`.
- **Public contract changed?** no new required JSON fields. Nests keep `how`/`do`; add `k03_source`. House map shrinks from 12 keys to occupied-only.
- **Migration required?** no — empty houses already omitted in UI when how is missing; encyclopedia HOUSE_FALLBACK no longer fills life-map / full-map K03
- **Canon updated?** yes — Information Contract §3/§11 · Display Inventory `P6.applied.*` · Experience Scenario applied emit volume · this tracker · handoff
- **Backward compatible?** yes for path P1–P5 (K02 Why no longer reads K03 how). Explore map shows fewer house theses when unoccupied.
...
- **SoT before:** `P4.sphere.*` came from identity-thesis life_spheres packs (love/money/work essays). F06 house arena was not the path selector. Grammar emit did not ship `P4.sphere.*`.
- **SoT after:** ≤2 path spheres from grounded full-natal F06 occupied houses of already built K01 occupancy / K04 axis / K05 pair, composed with IL-2 house `arena`. Sphere is where that mechanism shows. No new personality root. Identity-thesis / Stage4 / trap-bank cannot fill empty. K08 stays `nodes[0].help` → effort_vector. Missing F06 or mechanism link omits.
- **Public contract changed?** no new JSON fields; `profile_contract_v1.life_spheres` on the path is 0–2 F06 projections instead of 7 identity essays
- **Migration required?** no
- **Canon updated?** yes — Information Contract §3/§11 · Display Inventory `P4.sphere.*` · this tracker · handoff
- **Backward compatible?** yes for API. Date-only / no occupied F06 keep empty path spheres (omit)

## Architecture impact — PIC-K09 honest cost (2026-09-20)

- **SoT before:** K09 was Stage4 `blind_spots` / identity-thesis trap essay; consumption did not read it. P3.insight = K05 only; P3.help = K04 only.
- **SoT after:** One honest cost derived from grounded K04 F08 axis (sign `canon.excess`) plus grounded K05 A↔B. Fill-empty append onto `P3.insight`. K04 remains `P3.help`. K05 remains the A↔B owner. No trap-bank, no Stage4 LLM, no new slot. Harmonic-only K04 or missing pair omits.
- **Public contract changed?** no new JSON fields; `nodes[0].insight` may include a second excess beat
- **Migration required?** no
- **Canon updated?** yes — Information Contract §3/§11 · Display Inventory `P3.insight` · this tracker · handoff
- **Backward compatible?** yes for API. Charts without both hops keep previous insight/help.

## Architecture impact — PIC-K04 Internal Engine axis (2026-09-20)

- **SoT before:** Stage 3 Internal Engine filled seven identity-thesis slots; path help came from growth/recovery essays. F08 did not exist. F07 lived in Stage 0 for K05 only.
- **SoT after:** Stage 0 mints `element_balance` (F08). One path axis from unique F08 tilt (IL-2 sign `how`) or one harmonic F07 (`relation`, not A↔B). Consumption writes that line to `P3.help`. Identity-thesis widgets cannot occupy the path axis. K05 keeps `P3.insight`. Empty omits.
- **Public contract changed?** no new JSON fields; `nodes[0].help` source may now be F08/harmonic lemmas
- **Migration required?** no
- **Canon updated?** yes — Information Contract §3/§11 · Display Inventory `P3.help` · this tracker · handoff
- **Backward compatible?** yes for API. Charts without unique F08/harmonic keep previous help fill (K10 still PARTIAL)
...
- **Backward compatible?** yes for JSON shape. Two same-Sun charts can now differ on first-paint recognition when occupancy differs.
...
- **SoT before:** Production MY DAY could take Priority from Glance Daily Focus `prioritize` when that line equaled `today_move`. Glance synthesizer mixed do[0] / today_move / expect / guide lines. Screen 0 was not a journey act, but still sat on the composition path.
- **SoT after:** Glance is not a fifth act and not an importance picker. `T3.priority` = Personal `do[]`, else persisted `today_move`. `buildGlanceDailyFocus` is legacy `?experience=1` only. `ProfileCharacterScene` removed from the repo.
- **Public contract changed?** no JSON.
- **Migration required?** no. Empty Priority omits.
- **Canon updated?** yes — `DISPLAY_CONSTRUCTION_GRAMMAR_V1` §5 · Today Inventory T3.priority / §4 · TODAY_PRODUCT_FLOW_V1 · this tracker.
- **Backward compatible?** yes for API.
...
## Architecture impact — Content Library Selection v1 (2026-08-31)

- **SoT before:** Content Library had a validator, coverage ledger, and lightweight fill process, plus a declared selector service that was not yet implemented. The hub (`GET /practices`) and detail (`GET /practices/{id}`) used only the legacy `GENERAL_PRACTICES` in-memory catalog (47 free items). Meaning surfaces either used legacy catalog data or fell back to hardcoded placeholders.
- **SoT after:** `content_library_selection_v1.py` is now implemented and wired into `api/practices.py`:
  - `GET /practices` merges active/accepted Content Library items into the hub catalog.
  - `GET /practices/{id}` falls back to Content Library when a static or personalized practice is not found.
  - `GET /practices/select` remains the canonical deterministic selector endpoint (purpose/direction hard tags + soft ranking). It loads `content_library_v1.json` and `technique_canon_v1.json`, filters for `active` items whose `technique_id` is `accepted`, matches hard tags, and ranks by soft tag overlap (`input_state`, `context`, `energy_effect`, `duration`). The sort key is stable and reproducible. Meaning still does not emit `item_id` or `technique_id`.
- **Public contract changed?** yes — `GET /practices` now returns additional free catalog items derived from Content Library; response schema fields unchanged, but `id` values are now content-library `item_id`s. `GET /practices/{id}` may resolve content-library ids. `GET /practices/select` unchanged.
- **Migration required?** no runtime data migration. Content Library items are read-only from the deployed `DATA` directory.
- **Canon updated?** yes — `docs/practices/CONTENT_LIBRARY_SELECTION_V1.md` · `docs/practices/PRACTICE_LIBRARY_FILL_V1.md` · this tracker.
- **Backward compatible?** yes for existing IDs. All new IDs are additive; old IDs continue to resolve.
...
**NOW (PRACTICES / CANON, 2026-08-31):** **Content Library wired to the hub.** `content_library_selection_v1.py` is implemented and merged into `GET /practices` and `GET /practices/{id}`. The hub catalog now includes all 111 active Content Library items with accepted techniques alongside the 47 legacy free practices. Items are mapped to public `need_ids`, `format_id`, and `category` for the existing state-cycle filters. `GET /practices/select` deterministic selector is live. P0 type coverage milestone remains: 40/44 P0 spine types sourced, 4 skipped. Next: density pass (additional duration/context/delivery expressions) or reassess skipped types when a product requirement demands them. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md) · [CONTENT_LIBRARY_SELECTION_V1](./practices/CONTENT_LIBRARY_SELECTION_V1.md). Meaning still does not emit `item_id` or `technique_id`.
...
- **SoT after:** fill = lightweight provenance (source check → own-words description → safety notes if material → accepted/skipped → Content Item). Research ladder = archive, non-blocking. `box_breathing`, `energizing_breath`, `abstinence`, `self_trust` = `skipped_for_now`. All 26 P0 need cells sourced; P0 type coverage now active. 40/44 P0 spine types sourced; all non-skipped P0 spine types are sourced. Content Library is now wired to the hub via `content_library_selection_v1.py` (111 active items served). Next Product = density pass (duration/context/delivery expressions) or reassessment of skipped types when a product requirement demands them. 22 items remain `draft` (skipped types). Taxonomy classes/types, coverage cells, item contract field groups, and Meaning boundary unchanged.
- **Public contract changed?** yes — `GET /practices` / `GET /practices/{id}` now include Content Library items; response schema unchanged, IDs additive.
- **Migration required?** no runtime. Existing items stay provisional without `technique_id`.
- **Canon updated?** yes — [PRACTICE_LIBRARY_FILL_V1.md](./practices/PRACTICE_LIBRARY_FILL_V1.md) · [CONTENT_LIBRARY_SELECTION_V1.md](./practices/CONTENT_LIBRARY_SELECTION_V1.md) · [PRACTICE_TECHNIQUE_RESEARCH_ARCHIVE_V1.md](./practices/PRACTICE_TECHNIQUE_RESEARCH_ARCHIVE_V1.md) · provenance 1.12 · technique canon contract 1.1 · coverage next_pass · landscape 1.6 pointer · `_INDEX` · README · this tracker
...
- **SoT before:** Jargon and seed-kill retries were separate essays. Prod gen **1117** (user 15) oscillated `ASTRO_JARGON_BARE` on `astrology[1]` ↔ `verbatim_seed_leak` (why_today pasted into `human_meaning`). Detectors unchanged.
- **SoT after:** **Native C1 Astro Jargon Retry V1** — prompt `day-scenario-native-c5.4`; both retries carry `SEED_JARGON_CROSS_HINT_RU` (every `astrology[i]`; no why_today paste; no jargon rollback). Public JSON unchanged.
- **Public contract changed?** no
- **Migration required?** no — force rebuild / refresh picks up c5.4
- **Canon updated?** yes — `docs/today/NATIVE_C1_ASTRO_JARGON_RETRY_V1.md` · tracker 1.3.121
- **Backward compatible?** yes — same blocking codes

## Architecture impact — Today 1.3.120 Native C1 seed leak retry (2026-08-24)

- **SoT before:** Seed-kill ran only after I0 merge. Prod gen **1101** cloned sky-fact setup (`verbatim_seed_leak`); gen **1116** mapped sky-fact `conflict.title` (`conflict_short_name_is_sky_fact`). Detectors unchanged; no Global retry.
- **SoT after:** **Native C1 Seed Leak Retry V1** — same detectors on native Global; hard native markers + retry coaching; prompt `day-scenario-native-c5.3`. Public JSON unchanged.
- **Public contract changed?** no
- **Migration required?** no — force rebuild / refresh picks up c5.3
- **Canon updated?** yes — `docs/today/NATIVE_C1_SEED_LEAK_RETRY_V1.md` · tracker 1.3.120
...
- 2026-09-19 | Practices / Canon | **Library fill: meditation.loving_kindness sourced** | **ACCEPTED** | P1 type `meditation.loving_kindness` via `technique.loving_kindness` (NHS Scotland Right Decisions actual practice + NHS Lothian Loving Kindness meditation + BSMHFT sending kind thoughts). Brief well-wishing: self then one person; not self_compassion/connection_action/meditation.gratitude. Not full MBSR four-circle protocol, CFT, or treatment. `meditation.loving_kindness.001` active. Next = meditation.self_compassion. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md).
...
- 2026-09-17 | Practices / Canon | **Library fill: boundary_action sourced** | **ACCEPTED** | P1 type `practice.boundary_action` via `technique.boundary_action` (NHS Every Mind Matters work-related stress + Mind Practise saying no + Leicestershire Partnership NHS Assertiveness). Brief one decline; not micro_action/connection_action/digital_pause/digital_limit. Not assertiveness curriculum, broken-record, rights lists, or treatment. `practice.boundary_action.001` active. Next = practice.drawing. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md).
...
- 2026-08-31 | Practices / Canon | **Library fill: shaking_release sourced** | **ACCEPTED** | P1 type `practice.shaking_release` via `technique.shaking_release` (Cardiff and Vale UHB NHS Wales sedentary-worker micro-pause + West Suffolk NHS loosening + UMass Memorial workstation arm shake). Brief voluntary shake-out; not body_release/mobility/stretch/TRE. `practice.shaking_release.001` active. Next = practice.gratitude. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md).
- 2026-08-31 | Practices / Canon | **Library fill: walking sourced** | **ACCEPTED** | P1 type `practice.walking` via `technique.walking` (NHS sit less + Better Health get active + NHS England corridor/2-minute walk). Brief locomotion sit-break; not mobility/mindful_movement/walking_meditation. `practice.walking.001` active. Next = practice.shaking_release. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md).
- 2026-08-31 | Practices / Canon | **Library fill: mindful_movement sourced** | **ACCEPTED** | P1 type `practice.mindful_movement` via `technique.mindful_movement` (Sussex Mindfulness Centre NHS + UHDB NHS + RUH Bath PHY051). Brief movement-with-attention; not mobility/stretch/sit-mindfulness/walk. `practice.mindful_movement.001` active. Next = practice.walking. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md).
- 2026-08-31 | Ops / Deploy | **Live compose cutover + disk rescue** | **LIVE** | Recreated backend+frontend on `todayflow.today` from `1166ab96` + leftover practice/Grammar files now on `main` (no workflow file). Disk filled (docker images/cache); pruned 38 GB; postgres recovered after `No space left`. Health 200. Latch still tripped.
...
- 2026-08-31 | Today / FE | **Glance authority** | **CODE** | Glance synthesizer off production path. T3.priority = `do[]` else persisted `today_move`. `ProfileCharacterScene` deleted. Tests: `todayMyDayPriority` + live frames. [DISPLAY_CONSTRUCTION_GRAMMAR_V1](./foundation/DISPLAY_CONSTRUCTION_GRAMMAR_V1.md) §5.

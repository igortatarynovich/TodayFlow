...
**NOW (P1 PRACTICE LIBRARY FILL, 2026-08-31):** P0 fill complete: all 26 P0 need cells sourced, 40/44 P0 spine types accepted (`box_breathing`, `energizing_breath`, `abstinence`, `self_trust` skipped). P1 coverage: 2/42 sourced — `practice.paced_breathing` (NHS Ready to Relate + Harvard Health) and `practice.physiological_sigh` (Vlemincx et al. 2010 + KU Leuven resetter hypothesis + instructed-sigh relief). Both have active `.001` items with `technique_id`; coverage matrix now 135 library items. Content Library hub serves 113 active/accepted content-library items + legacy catalog. Tests green: `test_content_library_v1.py` (53), `test_content_library_selection_v1.py` (13), `test_practices.py` (27). Mapping updated: `paced_breathing` and `physiological_sigh` → category `breathing`, format `breath`. Next: `practice.grounding` (next P1 type in ledger order).

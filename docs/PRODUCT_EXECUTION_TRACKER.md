...
| DE-13 | **Narrative multi-call pipeline** (узкие шаги вместо одного guide на всё) | **DONE** | **v0–v4:** funnel interpretation → core → satellites; per-step cache; child chain; step3 core. **v5 (2026-07-03):** `guide_contract_v2` + `guide_pipeline_v0` в HTTP guide; `guide_funnel_core_source`; preserve LLM core vs `guide_decision`; web/iOS/Android parsers. Эпик DE-13 закрыт — монолит только fallback. |

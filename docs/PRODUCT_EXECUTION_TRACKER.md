...
**NOW (PRACTICES / CANON, 2026-08-30):** **Library fill** — next cell `need.reset.release`. Sourced 23/26. Latest: `need.simplicity.release` via `reduction` (select one unnecessary or attention-competing thing and remove/defer it for a short period; Paas & van Merriënboer 2020 cognitive-load theory + RACGP clutter/cognitive resources + Sweller et al. 2019 extraneous attention-capturing stimuli). `box_breathing`, `energizing_breath`, `abstinence`, and `self_trust` skipped. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md). Meaning still does not emit `item_id` or `technique_id`.

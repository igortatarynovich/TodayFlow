...
**NOW (PRACTICES / CANON, 2026-08-30):** **Library fill** — next cell `need.self_control.stabilize`. Sourced 19/26. Latest: `need.discipline.prepare` via `routine_commitment` (one small action at same time/cue for a short streak; NICE PH49 small routine changes + Lally & Gardner (2012) making health habitual + Keller et al. (2021) routine/time cue planning). `box_breathing`, `energizing_breath`, and `self_trust` skipped. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md). Meaning still does not emit `item_id` or `technique_id`.
...
- **SoT after:** fill = lightweight provenance (source check → own-words description → safety notes if material → accepted/skipped → Content Item). Research ladder = archive, non-blocking. `box_breathing` = `skipped_for_now`. Next Product = library fill at `need.self_control.stabilize`. 133 items remain `llm_provisional`. Taxonomy classes/types, coverage cells, item contract field groups, and Meaning boundary unchanged.
- **Public contract changed?** no

...
**NOW (BUDGET HOLD & PHASE 1 START, 2026-08-31):** Owner decision: Token Factory top-up is deferred until the app is ~90% complete. G0 billing is on hold; G1 (CI green) and G2 (ship gate) must proceed using deterministic fallbacks, mocked LLM, and local fixtures. All live-provider checks (`POST /chat/completions`, real COGS baseline, K3 Profile Selection audit) are deferred. Phase 1 Hardening launch-critical regressions now in progress, no live LLM required: (1) account delete cascade (`test_account.py` delete profile leaves orphaned `cached_natal_charts`); (2) Today contract assembler / text-quality fallback (`MONEY_ACTION_FALLBACK` fails imperative-action gate P0.1, 11 tests); (3) Compatibility guest smoke surface (guest tier returns 1 block instead of canon ≥4); (4) day-engine knowledge wiring (`select_knowledge_context_v1` threshold drops valid fixture facts, 17 tests). LLM-dependent infra/env tests (Tarot spread, Compatibility deep, forecasts) to be quarantined with mocks or xfail. Release plan updated to v1.1a; `ops_reset_llm_latch.py` env-path fix already in working tree but not committed. Next: pick and fix first Phase 1 regression.

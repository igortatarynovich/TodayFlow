...
**NOW (P1 SHAKING RELEASE, 2026-08-31):** P1 type `practice.shaking_release` sourced via `technique.shaking_release` (Cardiff and Vale UHB NHS Wales sedentary-worker micro-pause + West Suffolk NHS loosening + UMass Memorial workstation arm shake): drop arms, point hands to the floor, gently shake wrists/hands a few seconds, stop. Distinct from body_release, mobility, stretching, mindful_movement, and TRE. `practice.shaking_release.001` active. Coverage 140 items, 7/42 P1 sourced. Next: `practice.gratitude`.

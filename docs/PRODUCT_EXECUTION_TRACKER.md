...
**NOW (PRACTICES / CANON, 2026-08-30):** **P0 type coverage** — all 26 P0 need cells sourced; now filling P0 spine types. Latest: `practice.evening_ritual` accepted (NHS Every Mind Matters + Imperial NHS sleep hygiene): dim light, put device out of reach, sit, take a few slow out-breaths as a wind-down transition. `practice.evening_ritual.001/002/003` active with `technique_id`. P0 spine type coverage: 30/44 sourced. `box_breathing`, `energizing_breath`, `abstinence`, and `self_trust` remain skipped. Next non-skipped P0 spine type: `meditation.breath_awareness`. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md). Meaning still does not emit `item_id` or `technique_id`.
...
- **SoT after:** fill = lightweight provenance (source check → own-words description → safety notes if material → accepted/skipped → Content Item). Research ladder = archive, non-blocking. `box_breathing`, `energizing_breath`, `abstinence`, `self_trust` = `skipped_for_now`. All 26 P0 need cells sourced; P0 type coverage now active. 30/44 P0 spine types sourced. Next Product = next non-skipped P0 spine type (`meditation.breath_awareness`). 133 items remain `llm_provisional`. Taxonomy classes/types, coverage cells, item contract field groups, and Meaning boundary unchanged.
- **Public contract changed?** no

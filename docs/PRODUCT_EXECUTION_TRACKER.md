...
- 2026-07-25 | Profile / Architecture | **CE Stage 2 Identity Core** | **IN_PROGRESS** | LLM-first prompt `profile.character_engine.stage2.v1` · code = JSON/provenance gates only · flags `STAGE2_SHADOW`/`ENABLED` diagnostics-only · `PUBLISH_READY` reserved. **Baseline SHA:** `5eb61c6` (`feat(character-engine): add LLM-first identity core shadow`; CE-only). Later branch commits (Day Scenario / Tarot / etc.) do not change this CE baseline. **No** quality heuristics · **no** Stage 3–5 · **no** ready publish. Next: live Stage 0–1 shadow review → then Stage 2 shadow separately.
- 2026-07-25 | Profile / Architecture | **CE Stage 0–1 staging eval** | **GATE PASS** | Fixed fixtures · registry tightened · [CHARACTER_ENGINE_STAGE01_STAGING_EVAL_V0.md](audits/CHARACTER_ENGINE_STAGE01_STAGING_EVAL_V0.md). **Baseline:** `5eb61c6` — staging + Stage 2 results refer to this SHA, not branch tip. **Live recipe (Stage 0–1 only):** `STAGE01_SHADOW=1` · `STAGE01_ENABLED=0` · `STAGE2_SHADOW=0` · `STAGE2_ENABLED=0` · `PUBLISH_READY=0`.
...
- 2026-07-25 | Today / Scenario | **Day Scenario SoT + product capture** | **IN_PROGRESS** | Canon [DAY_SCENARIO_V1.md](./DAY_SCENARIO_V1.md) · **B1–B5 + C1 + C2 + C3.1 landed** (everyday scene quality + editorial gate) · [C3.1 note](./audits/DAY_SCENARIO_EVERYDAY_QUALITY_C31.md) · **next:** C3.2 chorus · C3.3 personalization · C3.5 eval pack

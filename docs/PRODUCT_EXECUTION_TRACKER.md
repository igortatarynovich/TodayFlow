...
**NOW (PRACTICES / CANON, 2026-08-30):** **Library fill** — next cell `need.habit_change.prepare`. Sourced 25/26. Latest: `need.presence.stabilize` via `mindfulness` (brief attention to present-moment sounds/sensations/breath, notice without judgment, return when mind wanders; Oxford Mindfulness definition + NHS mindfulness + Bishop et al. 2004). `box_breathing`, `energizing_breath`, `abstinence`, and `self_trust` skipped. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md). Meaning still does not emit `item_id` or `technique_id`.

...
**NOW (PRACTICES / CANON, 2026-08-26):** **Library fill** — next cell `need.focus.focus`. Preferred type `box_breathing` is `skipped_for_now`: pick another suitable type for that need, or skip the cell. Do **not** reopen box research. `need.calm.downregulate` is sourced (`technique.extended_exhale` accepted; `practice.extended_exhale.001` attached). [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md). Meaning still does not emit `item_id` or `technique_id`.

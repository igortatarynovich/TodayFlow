...
**NOW (PRACTICES / CANON, 2026-08-30):** **Library fill complete** — all 26 P0 need cells sourced. Latest: `need.habit_change.prepare` linked to existing `technique.consistency_challenge` (seven-day replacement rule: when the old pull comes, do one new short action in its place). `box_breathing`, `energizing_breath`, `abstinence`, and `self_trust` remain skipped. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md). Meaning still does not emit `item_id` or `technique_id`. Next: finalize skipped types or address non-P0 cells.

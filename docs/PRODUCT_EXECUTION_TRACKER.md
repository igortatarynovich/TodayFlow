...
- 2026-07-29 | Profile / UI | **Natal wheel CSS material pass** | **IN BRANCH** | Lit planet discs + drop shadow · aspect weight hierarchy (`natalWheelMaterial`) · soft→strong paint order · light CSS plate tilt + layer parallax · `prefers-reduced-motion` off · no WebGL · next: motion accents (Decode breathe / one-shots) then optional 3D go/no-go

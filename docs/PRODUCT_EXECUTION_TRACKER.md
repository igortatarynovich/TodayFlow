...
- 2026-07-30 | Today / Content | **Wave 2 Phase D.1 — `GET /today/day-facts`** | **PARTIAL LIVE** | Slot envelope: one `assemble_day_facts_v1` → natal_activations + domain_verdicts + glance_timeline + provenance. Interim `/domain-verdicts` + `/glance-timeline` = thin slices. FE `fetchDayFacts` once in GlanceAct; slots prefer parent payload. D.1b (conflict/scenes) · D.2–D.4 next. [CONTRACT](./today/TODAY_WAVE2_CONTRACT_V1.md) · [PLAN D.1](./today/TODAY_WAVE2_EXECUTION_PLAN.md).

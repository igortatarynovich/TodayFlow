...
**NOW (PRACTICES / CANON, 2026-08-30):** **P0 type coverage milestone reached** — all 26 P0 need cells sourced; all non-skipped P0 spine types now have sourced active items. Latest: `discipline.consumption_limit` accepted (The Independent 48-hour rule + Shah et al. 2023 planning prompts): for a short streak (7/14 days), defer any unplanned purchase not on today's plan and put it off for later review. `discipline.consumption_limit.001/002/003` active with `technique_id`. P0 spine type coverage: 40/44 sourced (remaining 4 are skipped: `box_breathing`, `energizing_breath`, `abstinence`, `self_trust`). Next: decide whether to begin P1 type coverage, return to density, or reassess skipped types based on product need. [PRACTICE_LIBRARY_FILL_V1](./practices/PRACTICE_LIBRARY_FILL_V1.md). Meaning still does not emit `item_id` or `technique_id`.
...
- **SoT after:** fill = lightweight provenance (source check → own-words description → safety notes if material → accepted/skipped → Content Item). Research ladder = archive, non-blocking. `box_breathing`, `energizing_breath`, `abstinence`, `self_trust` = `skipped_for_now`. All 26 P0 need cells sourced; P0 type coverage now active. 40/44 P0 spine types sourced; all non-skipped P0 spine types are sourced. Next Product = decision on P1 type coverage, density, or reassessment of skipped types. 133 items remain `llm_provisional`. Taxonomy classes/types, coverage cells, item contract field groups, and Meaning boundary unchanged.
- **Public contract changed?** no

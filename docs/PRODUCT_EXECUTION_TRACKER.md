...
- 2026-08-01 | Astro / Foundation | **Foundation v1 (geometry + constants + routing)** | **IN PROGRESS** | Geometry closed. Next: atomic constant tables (astro+numerology) → consolidate tarot/color SoT → formulas lookup (no valence literals) → then hooks/semantics. Canon [foundation_v1.md](./foundation_v1.md). Quincunx remains OOS v1.
- 2026-08-01 | Astro / Foundation | **Geometry closeout (coords/Swiss/angles)** | **DONE** | Swiss `FLG_SWIEPH` live (`astro/ephe`, no silent Moshier / 503); geocode small towns + ambiguous `need_choice`/409; Igor/Kyiv re-warmed; Einstein ASC/MC vs Astrotheme ≤1′; taps untouched. Canon [GEOMETRY_FOUNDATION_CLOSEOUT_V1](./audits/GEOMETRY_FOUNDATION_CLOSEOUT_V1.md). Parent [foundation_v1.md](./foundation_v1.md).

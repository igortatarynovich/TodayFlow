...
**Статус:** промежуточный входной контракт (`day_context_v0`) для `POST /today/narrative`. **Не Meaning SoT.** Смысл дня — только [today/TODAY_CONTENT_PIPELINE_V1.md](./today/TODAY_CONTENT_PIPELINE_V1.md). DayModel ([DAY_ENGINE_AND_COHERENCE.md §10](./DAY_ENGINE_AND_COHERENCE.md#10-daymodel-и-decision-engine-закрытый-канон)) — не канон смысла.  
...
`DayContext` — один логический пакет фактов и сжатых интерпретаций для **дня** (`target_date`), из которого собираются промпты narrative, объяснимость UI и (далее) события обучения. Это **вход** в сборщик, не решение смысла. v0 **не меняет** форму ответа `today_day_v1` и payload surfaces (**DE-2** — встроен в `build_today_narrative`, см. ниже).
...
| character_continuity | `character_continuity` | **CE §3.1:** то же `identity_line` + `primary_tension`; `day_angle` ротируется по дате (`character_engine_day_angle_v0`). Запрет нового героя. В LLM pack через `_attach_profile_slices`. |
...
| DayModel (§10) | `day_model` | **`day_model_v0`** — детерминированный объект для UI/объяснимости и срезов LLM (**не** Meaning SoT; смысл дня = [TODAY_CONTENT_PIPELINE_V1](./today/TODAY_CONTENT_PIPELINE_V1.md)). **DE-9 v1.4:** при `history_slice` — поле `temporal`. |

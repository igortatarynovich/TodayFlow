...
| 1.6 | Parallel: Practice Library fill | +1 sourced P0 cell per week starting from `need.connection.connect` | 0.1 |
...
| # | Task | Acceptance Criteria | Depends On |
|---|------|---------------------|------------|
| 3.1 | Practice Library 26/26 sourced | `content_coverage_matrix_v1.json`: 26 covered, 0 seed; `content_library_v1.json` not `llm_provisional` — **13/26 covered 2026-08-29** (`need.self_connection.reflect` sourced via `technique.journaling`; next cell `need.connection.connect`) | 1.6 |
...
|-------|----------|---------------|
| Practice Library fill | 1 cell / week | Phase 0 | // now at 13/26, next `need.connection.connect`

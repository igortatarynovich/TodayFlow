...
|---|------|---------------------|-------|
| 0.1 | Merge current practice fill changes | `test_content_library_v1.py` green; coverage counts 21/26 sourced | Backend + Content |
...
| 1.6 | Parallel: Practice Library fill | +1 sourced P0 cell per week starting from `need.consistency.prepare` | 0.1 |
...
| # | Task | Acceptance Criteria | Depends On |
|---|------|---------------------|------------|
| 3.1 | Practice Library 26/26 sourced | `content_coverage_matrix_v1.json`: 26 covered, 0 seed; `content_library_v1.json` not `llm_provisional` — **21/26 covered 2026-08-30** (`need.detachment.release` sourced via `technique.acceptance`; primary `abstinence` skipped; next cell `need.consistency.prepare`) | 1.6 |
| 3.2 | Finalize skipped techniques | `box_breathing`, `energizing_breath`, `abstinence`, `self_trust` — either accepted or permanently skipped with reason | 3.1 |
...
|-------|----------|---------------|
| Practice Library fill | 1 cell / week | Phase 0 | // now at 21/26, next `need.consistency.prepare`

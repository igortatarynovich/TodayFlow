...
| 2.6 | Parallel: Practice Library fill | P0 cells complete; next: finalize skipped types or address non-P0 cells | 0.1 |
...
| # | Task | Acceptance Criteria | Depends On |
|---|---|---|---|
| 4.1 | Practice Library 26/26 sourced | `content_coverage_matrix_v1.json`: 26 covered, 0 seed; all 26 P0 need cells sourced as of **2026-08-30** (`need.habit_change.prepare` linked to `technique.consistency_challenge`). Remaining `llm_provisional` items do not block launch. | 2.6 |
| 4.2 | Finalize skipped techniques | `box_breathing`, `energizing_breath`, `abstinence` — either accepted or permanently skipped with reason; `self_trust` already skipped | 4.1 |
...
|-------|----------|---------------|
| Practice Library fill | P0 cells complete | Phase 0 | // now 26/26 P0 sourced; next: finalize skipped types or non-P0 cells
...
| 2026-08-30 | Practice Library P0 fill complete: `need.habit_change.prepare` linked to `technique.consistency_challenge`; all 26 P0 need cells sourced; next: finalize skipped types or non-P0 cells. |

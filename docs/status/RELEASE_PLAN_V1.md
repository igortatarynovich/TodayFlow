...
| 1.6 | Parallel: Practice Library fill | +1 sourced P0 cell per week starting from `need.simplicity.release` | 0.1 |
...
| # | Task | Acceptance Criteria | Depends On |
|---|------|---------------------|------------|
| 3.1 | Practice Library 26/26 sourced | `content_coverage_matrix_v1.json`: 26 covered, 0 seed; `content_library_v1.json` not `llm_provisional` — **22/26 covered 2026-08-30** (`need.consistency.prepare` sourced via `technique.consistency_challenge`; next cell `need.simplicity.release`) | 1.6 |
...
|-------|----------|---------------|
| Practice Library fill | 1 cell / week | Phase 0 | // now at 22/26, next `need.simplicity.release`

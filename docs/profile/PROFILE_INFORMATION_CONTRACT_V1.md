...
- **SoT after:** этот файл — **закрытый перечень** фактов и допустимых знаний Profile. Display Inventory остаётся последним authority перед UI. Character Engine остаётся SoT композиции личности. IL остаётся SoT астрологических атомов. Строка вне таблиц §1–§3 **не существует** как содержание Profile.
- **Public contract changed?** no JSON
- **Migration required?** no
- **Canon updated?** yes — этот файл · `profile/_INDEX.md` · `docs/README.md` · tracker · handoff
- **Backward compatible?** yes for API. Новый запрет документарный, пока строка не используется как gate в коде.
...
|----|-----------|-------|------------|------------|----|------|------|-------|-----|--------|
| `K01` | Один наблюдаемый механизм личности (логлайн) | `F01`–`F04`, `F09`, full: `F05` | Planet `core_function` · Sign `manner` · (full) Angle `orientation` | CE Акт I: одна мысль из Evidence Graph. **Не** список черт, **не** sun-bucket сам по себе | IL 7 планет draft; CE Stage 1 = 13 sun/moon/ASC ключей | CE каскад есть; live Stage 1 **не** IL | **дырка:** два одинаковых Солнца → один шаблон | **да** | `P1.recognition_line` · `P1.identity_core` · Matrix `identity_summary` | 1 предложение механизма |
| `K02` | Роль каждого факта в **этом** ядре | subset `F01`–`F10`, `F12` | те же атомы; каталог не энциклопедия | CE Акт II: «какую часть этой личности я сформировал». Тест: без имени планеты фраза про человека | атомы есть; CE prologue декларирует китайский/тибетский как evidence — Stage 1 **не** матчит | правило есть | partial: `P2.anchor.*` банк по Солнцу/Луне/ASC/MC/числу | **да** | `P2.anchor.sun/moon/asc/mc/element/rhythm` · `P2.selected_life_path` | 1 тезис на опору; пустое omit |
...
| `K13` | Что имя-числа вносят в самопрезентацию | `F10` | тот же bank | Capability L1 `name_expression`; omit без имени | bank есть | канон: не влиять на натал | слой шапки Matrix; в Display Inventory **нет отдельного slot_id** | **да по Matrix / дырка Inventory** | шапка «Нумерология имени» | omit + CTA без имени |
| `K14` | Культурные соответствия знака/даты | `F12` | header pack · chinese/tibetan services · sign stones/colors | **lookup**, не LLM. Нет ключа → omit | цвета hardcoded; камни из sign catalog; год — сервисы | Matrix closed decision #10 | pack есть; Display Inventory не даёт slot «камень/цвет шапки» | **да по Matrix / дырка Inventory** | шапка «Соответствия» | ключ+ярлык; без выдумки |

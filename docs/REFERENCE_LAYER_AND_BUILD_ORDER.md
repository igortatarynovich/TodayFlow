...
| Practice | PracticeTemplate | `DATA/reference/practice/content_library_v1.json` (133 items; sourced calm + focus + energy) + `technique_canon_v1.json` (extended_exhale + focused_attention + mobility accepted; box_breathing + energizing_breath skipped) + legacy `CONTENT/practices/practices.json` | identity + retrieval + payload; optional `technique_id` only if accepted | title, body | Flow, Today recommendations | fill unfrozen; next = `need.grounding.stabilize` |

...
| Practice | PracticeTemplate | `DATA/reference/practice/content_library_v1.json` (133 items; sourced 11/26) + `technique_canon_v1.json` (11 accepted; box_breathing + energizing_breath + self_trust skipped) + legacy `CONTENT/practices/practices.json` | identity + retrieval + payload; optional `technique_id` only if accepted | title, body | Flow, Today recommendations | fill unfrozen; next = `need.emotional_awareness.reflect` |

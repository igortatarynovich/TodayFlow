...
1. **Library fill** — [PRACTICE_LIBRARY_FILL_V1.md](./PRACTICE_LIBRARY_FILL_V1.md). Sourced: `need.calm.downregulate`, `need.focus.focus`, `need.energy.activate`. Следующая ячейка: `need.grounding.stabilize`. `box_breathing` и `energizing_breath` = `skipped_for_now`. Не Safety Review V1.1.

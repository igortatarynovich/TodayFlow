# Practices — индекс канона

| Документ | Роль |
|----------|------|
| [PRACTICES_SCREEN_V1.md](./PRACTICES_SCREEN_V1.md) | **SoT** экрана «Практики»: цикл состояния, locked списки need/format, сессия, чек-ин, music layer, visual |

**Связанные (не заменяют этот пакет):**

- [TODAY_SCREEN_V1_CANON.md](../TODAY_SCREEN_V1_CANON.md) — практика дня / recommendation в Today
- [TODAYFLOW_FOUNDATION_UI.md](../TODAYFLOW_FOUNDATION_UI.md) — visual SoT (токены; Practices не вводит параллельный design-канон)
- [REFERENCE_LAYER_AND_BUILD_ORDER.md](../REFERENCE_LAYER_AND_BUILD_ORDER.md) — Practice / Habit / Ascetic registries
- Legacy catalog keys в коде: `frontend/src/components/today/practicesCatalogContent.ts` — **мигрировать** к locked IDs из PRACTICES_SCREEN_V1; не расширять старые goal-keys как SoT навигации

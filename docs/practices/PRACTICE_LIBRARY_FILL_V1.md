...
| Дата | Изменение |
|------|-----------|
| 2026-09-18 | P1 type `meditation.visualization` sourced via `technique.visualization` (NHS Inform Visualisation exercise + CUH NHS guided imagery + Oxford Health CAMHS Visualisation). Brief calm-place image: sit or lie still, picture an ordinary restful place, notice a few sensory details, stop. Distinct from practice.visualization (mental rehearsal), future_self, and intention_setting. Not full clinical guided-imagery protocol, manifestation, or treatment. `meditation.visualization.001` active. P1 coverage: 16/42 sourced, 1 P1 skipped. Next: `meditation.loving_kindness`. |
| 2026-09-17 | P1 type `practice.release_ritual` sourced via `technique.release_ritual` (ELFT NHS Dealing with Worry + MindWell Leeds Taking control of stress + University of Exeter CEDAR Problem Solving). Brief write-and-aside close: one line you are done carrying, strike through or fold/crumple and set aside or bin, stop. Distinct from transition_ritual, evening_ritual, and meditation.letting_go. Not full worry-time protocol, fire rites, or treatment. `practice.release_ritual.001` active. P1 coverage: 15/42 sourced, 1 P1 skipped. Next: `meditation.visualization`. |
| 2026-09-17 | P1 type `practice.music` sourced via `technique.music` (NHS Every Mind Matters low mood + sleep tips + Mind Relaxation Listen to music). Brief one-track listen: pick a short known track, play once, listen only to that, stop. Distinct from drawing, creative_prompt, and session accompaniment. Not music therapy, playlist programme, or sleep protocol. `practice.music.001` active. P1 coverage: 14/42 sourced, 1 P1 skipped. Next: `practice.release_ritual`. |
| 2026-09-17 | P1 type `practice.drawing` sourced via `technique.drawing` (CUH NHS Sketch Appeal + Calderdale and Kirklees Recovery College mindful drawing + Greater Good in Education Drawing as a Way to Manage Emotions). Brief freehand sketch: pen and paper, about two minutes on something ordinary or simple shapes, stop. Distinct from creative_prompt, free_writing, and music. Not art class, Zentangle curriculum, or art therapy. `practice.drawing.001` active. P1 coverage: 13/42 sourced, 1 P1 skipped. Next: `practice.music`. |
| 2026-09-17 | P1 type `practice.boundary_action` sourced via `technique.boundary_action` (NHS Every Mind Matters work-related stress + Mind Practise saying no + Leicestershire Partnership NHS Assertiveness). Brief one decline: pick one request you do not have room for, decline in one short line, stop. Distinct from micro_action, connection_action, digital_pause, and digital_limit. Not assertiveness curriculum, broken-record, rights lists, or treatment. `practice.boundary_action.001` active. P1 coverage: 12/42 sourced, 1 P1 skipped. Next: `practice.drawing`. |
| 2026-09-02 | P1 type `practice.future_self` sourced via `technique.future_self` (University of Edinburgh Reflection Toolkit + Greater Good Best Possible Self / King 2001 + Hershfield 2011). Brief later-self image: pick a later time, picture who you would be then in ordinary detail, stop. Distinct from intention_setting, visualization, and journaling. Not BPS writing protocol, letter programme, or manifestation. `practice.future_self.001` active. P1 coverage: 11/42 sourced, 1 P1 skipped. Next: `practice.boundary_action`. |
| 2026-08-31 | P1 type `practice.visualization` sourced via `technique.practice_visualization` (UHD NHS Mental Practice + Feltz & Landers 1983 + Holmes & Collins 2001). Brief mental rehearsal: pick one upcoming action, imagine doing it step by step without moving, stop. Distinct from intention_setting, future_self, and meditation.visualization. Not GMI/stroke rehab/manifestation. `practice.visualization.001` active. P1 coverage: 10/42 sourced, 1 P1 skipped. Next: `practice.future_self`. |
| 2026-08-31 | P1 type `practice.review` sourced via `technique.review` (NHS Transformation Partners End of Shift Wellbeing Checklist + UH Bristol reflective writing + NHS Borders look back). Brief two-pole look-back: name one hard thing and one thing that went okay or helped, then stop. Distinct from journaling, prompted_reflection, gratitude, self_check_in, and evening_ritual. Not Gibbs/AAR. `practice.review.001` active. P1 coverage: 9/42 sourced, 1 P1 skipped. Next: `practice.visualization`. |
| 2026-08-31 | P1 type `practice.gratitude` sourced via `technique.practice_gratitude` (NHS 10 stress busters + Every Mind Matters stress + Greater Good Three Good Things). Brief constrained list: write three things that went well or that you notice, then stop. Distinct from journaling, prompted_reflection, and meditation.gratitude. `practice.gratitude.001` active. P1 coverage: 8/42 sourced, 1 P1 skipped. Next: `practice.review`. |

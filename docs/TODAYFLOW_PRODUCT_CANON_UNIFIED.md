...
## 7. Монетизация — статус: гипотеза, не решение

**Важно (поправка 24.07):** валюта/цифры в текущем UI `pricing/page.tsx` (Free/990₽/2490₽) — **пример**, не зафиксированное решение. Ниже — актуальный канон, который явно говорит то же самое: цену рано фиксировать.
...
**Pricing:** не фиксировать до поля. Зависит от дизайна, Week 1 Report, D7 retention. Ориентир для A/B (не решение) — тестировать несколько ценовых точек по факту, не переносить их в прод как финал.

**Явное правило для launch:** монетизация — **не блокер** DoD запуска v1. Paywall + цена — отдельная волна после первых пользователей. Если экран `pricing/page.tsx` в текущем виде выглядит как финальное решение с фиксированными тарифами — это несоответствие канону, см. Cleanup Report.
...
| Тарифы фиксированы в ₽ в коде vs «цену рано фиксировать» в каноне | `pricing/page.tsx` vs `TODAYFLOW_PRODUCT_MODEL` §4.11 | §7 выше — код должен либо явно помечать цифры как placeholder, либо не показываться как финальный прайсинг до вол­ны 2 |
...
**Платежи:** открытый бизнес-вопрос (RU-эквайринг vs Stripe+$/€) — без решения не писать billing-код дальше §7.

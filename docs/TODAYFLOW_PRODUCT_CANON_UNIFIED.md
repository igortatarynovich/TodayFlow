# TodayFlow — Product Canon (Unified v1.0)
...
| **Maps** | Живая накопительная история жизни — вторая половина продукта | «трекер» / «статистика» |
...
| Половина | Вопрос | Слой |
|---|---|---|
| **Кто я** | Кто я? Почти не меняется | Portrait · natal · numerology · циклы · сильные/слабые · отношения · деньги · предназначение |
| **Как меняется моя жизнь** | Каждый день — история | **Maps** · «Мои дни» · shareable arcs |

North star: портрет + живая история, не CRM и не натальная стена. Launch v1: identity strip + «Мои дни» + MapsPreview seed + свёрнутые deep-слои.
...
affirmations · asceticisms · challenges · rewards · growth (полный) · library · maps (кроме «Мои дни» на Profile) · reports · weekly · discover · dashboard (уже redirect → `/today`)

Redirect-заглушки (`/dashboard`, `/horoscopes`, `/birth-chart`) **не удалять** — совместимость со старыми/deep-link URL.

**Платформа v1:** web-first. iOS параллельно, но не блокер web launch (см. `status/IOS_TODAYFLOW_STATUS.md`).

...
- **Обязательно в v1 коде:** персистить факт «user достиг тира N в дату X» (`ring_tier_reached_at` per user × tier) при пересечении порога. Дёшево сейчас; восстановить задним числом — нет. Пик индекса (`reward_evolution_index_peak`) уже есть — дополнить **датами по тирам**, не только списком earned ids.

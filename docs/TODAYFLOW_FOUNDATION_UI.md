...
| Поверхности · герои · символы · геометрия | [PROFILE_SCREEN_MASTER.md](profile/PROFILE_SCREEN_MASTER.md) (композиция экрана, не глифы) · декоративная эзотерика (совы, кристаллы, глаза, свечи, «дом денег») |
...
Это **системный визуальный язык**, который отражает данные продукта — не каталог ассетов «на каждый экран».

Смысл не живёт здесь: атомы и трактовки — [foundation_v1.md](./foundation_v1.md) §2 · [INTERPRETATION_LIBRARY_V1.md](./astrology/INTERPRETATION_LIBRARY_V1.md). Этот раздел — **как факт выглядит и как экраны из него собираются**.

**Правило данных:** нет рассчитанного факта — нет символа. Пустой слот / omit, не декоративная подстановка.

**Следствие:** после закрытия десяти слоёв (§2.1) потребность в новых иллюстрациях на экран резко падает. Profile · Today · Compatibility · натальная карта собираются из одного набора. Lucide / emoji / stock / совы / кристаллы / глаза / свечи / «дом денег» — запрещены.

### 2.0 Два регистра и два яруса глифов

**Регистры** (не смешивать в одном слоте):

| Регистр | Что | Где |
|---------|-----|-----|
| **Information** | глифы, типографика, геометрия | строки фактов, Today, Compatibility, натальная карта, chips |
| **Identity** | изображения небесных тел (единый стиль, не NASA-реализм) | XL hero, переходы, shareable — **никогда** не несёт информацию вместо Information |

**Исключение — Луна (§2.7):** единственный небесный объект, которому можно быть **информацией** (реальная фаза) и сферой с объёмом/светом одновременно. LRO-текстура допустима. Это не «фото планеты как иконка» и не разрешение NASA-crop для остальных тел.

Пользователь учится читать `♀ → ♉︎ → VII → △ → ♄` везде одинаково. Фото Венеры — визуальная сила крупного кадра, не замена глифа ♀ и не источник факта.

**Ярусы глифов** (как рисуем information-слой):

| Ярус | Что можно | Чего нельзя |
|------|-----------|-------------|
| **Canonical glyphs** | Перерисовать традиционный глиф элегантнее (единый stroke, text presentation) | Заменить пиктограммой «на тему» |
| **TodayFlow primitives** | Собственный геометрический / типографический язык | Притворяться традиционным астроглифом |

### 2.1 Десять слоёв (основа языка)

| # | Слой | Ярус / регистр | Форма | Правило |
|---|------|----------------|-------|---------|
| 1 | **Небесные тела** | information: глиф · identity: изображение | ☉ ☾ ☿ ♀ ♂ ♃ ♄ ♅ ♆ ♇ + restyle-набор картинок | Одна пластика, свет, детализация. Не фотореализм ради реализма. Глиф и изображение — разные слоты |
| 2 | **Зодиак** | canonical | 12 глифов ♈︎…♓︎ | Положение и архетип знака. Не иллюстрации знаков, не «портреты» |
| 3 | **Аспекты** | canonical | ☌ ☍ △ □ ⚹ | Графические **отношения** между объектами. `♂ □ ♀`, не иконка «конфликт» |
| 4 | **Дома** | primitive (type) | I–XII | Строгая типографика. `♀ · VII`. Никогда 12 картинок |
| 5 | **Углы** | primitive (type) | `ASC` `DSC` `MC` `IC` | Caps + tracking. Big Three: **☉  ☾  ASC**. Не пиктограммы, не photo-badge |
| 6 | **Доп. точки** | canonical | ☊ ☋ + только то, что войдёт в product canon | Узлы — расширенный профиль / долгие темы. Хирон / Лилит / PoF **не** семейства, пока не lock в foundation + IL |
| 7 | **Состояния и координаты** | canonical (type) | ℞ · `14°` · орбис · exactness | Маркеры факта, не украшение. Три слоя тела: объект ☿ · положение ♍︎ 18° · состояние ℞ |
| 8 | **Стихии и модальности** | geometry | △ ▽ △̅ ▽̅ · 3 примитива Cardinal/Fixed/Mutable | Только рассчитанный баланс. Не пламя/волны/ветер |
| 9 | **Циклы и фазы** | TF primitive | круг · дуга · кольцо · пересечение дуг | Положение человека в процессе (луна, Personal Year, Saturn, progressions). Geometry §3 G1/G2, не третьи кольца |
| 10 | **Нумерология** | TF primitive | фирменные 1–9 (+ 11/22/33) | Визуально родственны системе, достаточно отличны, чтобы читаться как **другой интерпретационный слой**. Методология не смешивается ([NUMBER_BASE_V1](./numerology/NUMBER_BASE_V1.md)) |

**Вне этих десяти** (не natal language): **Archetype** — identity art Hero Large (Profile Act 1 не меняется этим разделом). **Tarot** — [TAROT_DESIGN_LANGUAGE_V1](./tarot/TAROT_DESIGN_LANGUAGE_V1.md).

**Аспекты v1:** только пять мажоров — как [DAY_SOURCES_CANON](./DAY_SOURCES_CANON.md) и IL-1. Миноры foundation §2.4 в UI-язык не выводим. Квинконс — OOS.

**Стихии (алхимия):** △ Fire · ▽ Water · △ с чертой Air · ▽ с чертой Earth.  
**Модальности:** Cardinal = направление/начало · Fixed = центр/устойчивость · Mutable = движение/трансформация. Рисуются один раз в DS.

**Циклы:** круг = цикл · дуга = позиция внутри · кольцо = завершённый · две дуги = взаимодействие циклов.

### 2.2 Identity: изображения небесных тел

Отдельный набор, не глифы.

- Sun, Moon и 8 планет — **узнаваемые объекты** в одном стиле (пластика, свет, уровень детали).
- Не сток-NASA, не коллаж разных техник, не фото в круглом crop как «иконка».
- Слоты: XL hero · scene transitions · shareable. **Не** на строке факта, **не** на натальном диске как замена ♀.
- Identity создаёт узнаваемость. Information объясняет систему и масштабируется. Если изображение можно убрать и факт пропал — слот сломан.
- Текущие `planetPhotoPath` / `DsPlanet` photo — **переходные**. Restyle только внутри единого пакета §2.6.

### 2.3 Натальная карта — фирменный объект, не «астроколесо»

Карта **собирается из слоёв 1–7** (+ воздух Geometry §3). Это не отдельный дизайн и не традиционное колесо созвездий.

| Да | Нет |
|----|-----|
| Очень тонкая геометрия, минимум линий, много воздуха | Декоративные созвездия, эзотерический орнамент, starfield как декор |
| Знаки / тела / дома / аспекты = те же primitives, что везде | Свои «jewel»-диски, neon orbs, photo-planets на ободе |
| Аспект = линия/глиф отношения (☌ ☍ △ □ ⚹) | Толщина/цвет как единственный язык без глифа |
| Дома = I–XII на кольце | Арабские номера + иконки сфер |
| Углы = `ASC` `MC` type | Angle WebP badges |

Текущий `NatalChartWheel` (starfield, jewel discs, zodiac orbs, photo planets) — **переходный**. Карта и изображения тел **подчиняются** этому разделу; они не формируют параллельный стиль. Пересборка — только в пакете §2.6.

### 2.4 Синтаксис · грамотность поверхностей

Один слот = одно семейство. Не дублировать глиф иллюстрацией того же знака.

Цепочка читается одинаково в Profile, Today, Compatibility и на карте:

```
♀ → ♉︎ → VII → △ → ♄
тело  знак  дом  аспект  тело
```

Другие формы той же грамматики:
...
Mercury ℞      состояние
14° ♉︎         координата
△ 38%          стихия, только если % посчитан
7              нумерологический слой
○ / ◔ / ◑      цикл / фаза
```

### 2.5 Слоты, stroke, код

| Slot | px | Типичный регистр |
|------|-----|------------------|
| XL | 120 | identity (планета) · archetype |
| L | 80 | identity / крупный глиф |
| M | 48 | information |
| S | 24 | information · inline chain |

**Stroke (information):** 1.25px (S) · 1.5px (M–XL) · `--tf-ink-soft` / on-dark `--tf-on-dark`.  
Canonical glyphs: text presentation (U+FE0E). Unicode — только временный stand-in до SVG того же глифа.

**Код сегодня:** `design-system/visual/{DsPlanet,DsZodiac,DsNumber,DsAngle,DsCelestialMoon}` · `frontend/src/lib/visualIdentity/registry.ts`.  
**Переходные:** `DsAngle` photo-badge · `ElementIcon` пламя/волны · `DsPlanet` photo-as-icon · natal starfield/jewels.  
Feature не импортирует unicode/emoji напрямую.

**Figma (исторически):** `Symbols / Archetype` · `Zodiac` · `Element` · `Planet`. Новые семейства — в DS, не параллельный Figma-канон.

### 2.6 Единый пакет реализации (не независимые дизайн-задачи)

Не дробить на «сделать планеты», «сделать красивую натальную карту», «нарисовать глифы». Отдельный art-pass диктует собственный стиль. Нужна обратная зависимость: Natal и изображения тел подчиняются FOUNDATION_UI §2, а не задают язык.

Следующий art-pass — **один пакет**, один порядок:

1. **Visual grammar** — stroke, воздух, слоты, регистры Information / Identity.
2. **Glyph system** — знаки, аспекты, узлы, стихии, ℞ (canonical, один stroke).
3. **Typography / numerals** — `ASC` `MC` · I–XII · градус/орбис · фирменные 1–9.
4. **Celestial-body restyle** — identity-набор тел (после грамматики, не до).
5. **Natal composition** — тонкая карта из уже существующих атомов, не новый дизайн.
6. **Проверка на реальных экранах** — одна цепочка `♀ → ♉︎ → VII → △ → ♄` в Profile (не Act 1) · Today · Compatibility · карте.

**Запрещено до закрытия пакета:** отдельный planet restyle, отдельный natal polish, новые иллюстрации «под экран», трогать **Profile Act 1**.

### 2.7 Moon = live astronomical object

> Луна — не украшение. Она часть информационного интерфейса.

Единственный «космический» объект TodayFlow. **Никаких звёзд** (starfield, частицы-звёзды, сияющие созвездия, космическая пыль). Профильный layout «созвездие источников» (§ Profile) — метафора композиции фактов, не небо.

Состояние Луны определяется **реальными данными дня** (Swiss / day facts → `resolveCelestialMoonPhase`):

| Да | Нет |
|----|-----|
| Реальная фаза; освещённая часть диска совпадает с фазой | Декоративный диск «почти полная, потому что красиво» |
| Сферический объём и свет (терминатор, earthshine, rim) | Плоская иконка / сток-фото в круге |
| Плавное изменение фазы ото дня ко дню | Бесконечный idle-spin «планета крутится» |
| Код: `DsCelestialMoon` | Второй лунный рендер, Three.js-сцена, звёздный фон за сферой |

**Поверхности:**

| Где | Движение |
|-----|----------|
| **Лендинг** | Можно: очень медленная реакция на курсор/scroll **без смены фазы**. Фаза остаётся астрономической. **CODE:** визуальная подпись hero (`LandingSignatureMoon`). |
| **Приложение** | Почти статична. Допустимо одно короткое мягкое появление / лёгкий поворот сферы при открытии экрана. Не крутится постоянно. |
| **Share** | Статичный кадр фазы дня; можно чуть сильнее свет/объём, не анимационный loop. |

WebGL для Луны **уже есть** (`CelestialMoon`) — это и есть единственный разрешённый тяжёлый hero-объект. **Не** тянуть Three.js «для космоса». `prefers-reduced-motion` → без spin/loop, фаза рисуется кадром.

Движется информация, не декорация. То же правило на соседей: энергия дня → форма/градиент; таймлайн → положение во времени; meters → заполнение при появлении; аспекты → построение геометрии. Не «вау, что-то летает».
...
- **Цикл / орбита (§2.3):** G1/G2 кодируют положение в процессе (лунный цикл, Personal Year, Saturn). Не заводить отдельный набор «космических колец».
...
2. **Декоративный слой** — отдельно от фона, под контентом: линии, дуги, мягкие световые пятна. Не под текстом, не снижает читаемость. **Запрещены** частицы, starfield, бесконечно вращающиеся звёзды, сияющие созвездия, летающие знаки зодиака (§18).
...
4. **Motion** — только если объясняет состояние или усиливает настроение дня (§18). Атмосферный цикл, если есть: 15–40 сек, смещение на несколько px. Никаких быстрых бесконечных частиц. `prefers-reduced-motion` обязателен; вне активного экрана — стоп или упрощение.
...
| **Visual** | Единственный feature-импорт celestial UI | `design-system/visual/{DsPlanet,DsZodiac,DsNumber,DsTarotFace,DsAngle,DsCelestialMoon}` · Луна = live object §2.7 · язык семейств — §2 (aspect/house/node/degree/rx/modality/cycle — следующий DS-проход) |
...
- Каждая surface задаёт `color: var(--tf-ink)` — текст не сливается с атмосферой/луной.
- Sheet / modal / всё, что открывается **поверх** контента или imagery → только `tone="overlay"` (`DsOverlaySheet`) или `solid`. **Glass на оверлее запрещён.**
- In-page `glass` / `subtle` должны быть достаточно непрозрачны, чтобы body text читался поверх imagery.
...
## 18. Motion Dynamics — информация, не «вау»

**Статус:** ACCEPTED (2026-08-17).  
**Связь:** §2.7 Луна · §11.4 атмосфера · [TODAY_MOTION_PILOT_V1](./today/TODAY_MOTION_PILOT_V1.md) (внимание на Today: Tap / live-now — другой слой, не декор).

### 18.1 Принцип

> Анимация должна объяснять состояние интерфейса или усиливать настроение дня.  
> Если она существует только потому, что может двигаться — удалить.

React-украшения имеют смысл только там, где усиливают ощущение продукта. Не «типичный астрологический сайт»: частицы, бесконечно вращающиеся звёзды, сияющие созвездия, космический фон, летающие знаки зодиака. Это делает продукт дешевле и ломает направление **premium / wellness**, не эзотерика.

### 18.2 Бюджет по поверхностям

| Поверхность | Динамика | Что можно | Чего нельзя |
|-------------|----------|-----------|-------------|
| **Лендинг** | **7/10** | Лёгкий parallax, мягкие glow/gradient transitions, живые карточки Today, анимация натальной геометрии, смена «утро → день → вечер», micro-interactions на scroll. Луна — медленный pointer/scroll, фаза не врёт (§2.7). | Звёзды, созвездия, летающий зодиак, тяжёлый WebGL-космос |
| **Приложение** | **2–3/10** | Состояния, переходы, раскрытие карточек, динамика энергии дня, radial/meter fill, feedback на tap. Луна почти статична. | Постоянно двигающиеся декоративные объекты вокруг контента |
| **Share / result** | **5/10** | Сильнее, потому что кадр должны хотеть сохранить/выложить. Фаза Луны — факт дня. | Loop-декор, starfield «для сторис» |

Существующий idle `MotionDrift` на Profile / HeroLarge — **сверх бюджета приложения**. Не добавлять новые; снимать, когда экран в работе.

### 18.3 Стек

Не тянуть тяжёлую библиотеку ради украшений.

| Слой | Когда |
|------|--------|
| **CSS** | переходы состояний, meters, gradient, `prefers-reduced-motion` |
| **Framer Motion** (уже в репо) | enter/exit, карточки, scroll-linked на лендинге |
| **WebGL** | **только** `DsCelestialMoon` — сферическая Луна с реальной фазой. Не Three.js «сцена космоса» |

Один hero-объект на лендинге, если станет подписью TodayFlow = та же Луна, не новый шар со звёздами.

### 18.4 Живая информация (паттерн)

Движется смысл, не орнамент:

| Сигнал | Как двигается |
|--------|----------------|
| Фаза Луны | терминатор / объём сферы |
| Энергия дня | форма / градиент |
| Таймлайн | положение во времени |
| Meter | заполнение при появлении |
| Аспект | построение геометрии |

---

*Документ (§11) → контракты (§12) → реализация first pass (§13) → аудит (§14) → канон дизайн-системы (§15) → Today block-подача (§16) → миграция module.css (§17) → motion budget (§18). Figma нигде не участвует, кроме `figmaMap.ts` как справочной таблицы имён.*

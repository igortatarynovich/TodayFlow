...
Текущий `NatalChartWheel` (starfield, jewel discs, zodiac orbs, photo planets) — **переходный**. Пересборка — после (или вместе с) DS-примитивами, не параллельный art-pass.
...
**Следующий DS-проход:** restyle identity-планет · глифы знаков в одном stroke · `DsAspect` · `DsHouse` · `DsNode` · `DsDegree` / орбис / exactness · `DsRx` · `DsModality` · `DsCycle` · алхимический `DsElement` · пересборка `NatalChartWheel` из этих атомов. Feature не импортирует unicode/emoji напрямую.
...
| **Visual** | Единственный feature-импорт celestial UI | `design-system/visual/{DsPlanet,DsZodiac,DsNumber,DsTarotFace,DsAngle,DsCelestialMoon}` · язык семейств — §2 (aspect/house/node/degree/rx/modality/cycle — следующий DS-проход) |

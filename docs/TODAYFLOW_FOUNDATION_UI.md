...
| **Identity** | изображения небесных тел (единый стиль, не NASA-реализм) | XL hero, переходы, shareable |

Пользователь учится читать `♀ → ♉︎ → VII → △ → ♄` везде одинаково. Фото Венеры — эмоция крупного кадра, не замена глифа ♀.
...
- Текущие `planetPhotoPath` / `DsPlanet` photo — **переходные**. Restyle — один проход на весь набор.

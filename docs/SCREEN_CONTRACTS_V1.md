...
**Принцип:** человек сначала — ответ и шаг; карты и тарология — аргументы в конце (сворачиваемый «почему»). Не шесть независимых трактовок.

**Hard gates:** нет «Аркан»; `unresolved_cards` → technical fallback без LLM; Profile — короткий фрагмент в pack, не копипаст в body; `main_answer` / `today_suggestion` без таро-жаргона и имён карт.
...
| 2 | Ответ на вопрос | `main_answer` | Прямой, 1–2 фразы; без имён карт / мастей / «перевёрнут» |
| 3 | Что сделать дальше | `today_suggestion` | Один короткий проверяемый шаг |
| 4 | Выбор A/B (если choice) | `choice_story.option_*` | Параллельно и коротко: даёт / стоит |
| 5 | Оговорка (если есть) | `choice_story.confidence_note` | Одна строка: перепроверить опытом |
| 6 | Почему так (collapsed) | `symbols_overview` + `story_narrative` | Символы, карты, механизм — по желанию |
| 7 | Дальше | `next_routes` | Один контекстный CTA + optional save |

**Запрещено в `main_answer` / `today_suggestion`:** имена карт, масти, «Аркан», «перевёрнут*», «расклад», метафоры вроде «картина десятки кубков».

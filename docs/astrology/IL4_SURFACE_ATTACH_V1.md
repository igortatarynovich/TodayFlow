...
- Usage audit of which of the ~24 profile themes the model actually uses (K3 still blocked by billing)

Today *meaning polish* may resume as an **owner-directed** pass after consume 1.3.113. This attach file does not reopen.

---

## 3. Selection engine (product-side filter)

`services/il4_selection_v1.py` applies a deterministic, surface-aware filter on the IL-4 pack before it reaches the LLM prompt. It does not change IL-2/IL-3 ranking or meaning SoT.

| Surface | Rule | Default cap |
|---------|------|-------------|
| `today` | primary (rank 1) line only; transit-first sort already handled by IL-3 | 1 |
| `profile` | natal band only; optional topic filter; capped by rank | 24 |
| `compatibility` | natal + transit bands; optional topic filter; capped by rank | 24 |

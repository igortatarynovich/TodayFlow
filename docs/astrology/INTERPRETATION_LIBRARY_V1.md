...
**Freeze (методология IL):** не менять слои / evidence tiers / provenance shape до первых ~100 объектов (IL-1). **1.3.29–1.3.30** — исключения: author-first очередь и фиксация ядра от доступных книг. **1.3.45** — remaining planet budget = empty psychological slots. **1.3.57** — `ACCESS_BLOCKED(slot)`. **1.3.59** — planet fill research-stable / opportunistic; Layer 2 definition before bibliography. Следующее ядро: полный research order. Дальше — только баги модели.
...
**Stopped before step 5.** Schools, source types, literature map, shortlist, and ingest wait. Next agent on this track continues step 5 **from the model above**, not from an open Arroyo chapter.

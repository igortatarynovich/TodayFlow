...
**Next named:** ASC/MC definition (parent 1–4) — **done 1.3.99.** Next = Mainstream Angle Semantic Map. **STOP Aspects.** **STOP Houses.** **STOP Signs.**
...
**Next named:** Mainstream Angle Semantic Map (same panel). Not grammar. Not fill. Then grammar → fill → storage → angle smoke → final atomic smoke (Planet + Sign + House + Aspect + Angle) → Knowledge Core V1 FREEZE. After freeze: IL-2. **STOP Aspects.** **STOP Houses.** **STOP Signs.**
...
Не начинать с 10 000 комбинаций. Методология отбора смысла: **1.3.76–1.3.82** Mainstream → grammar → packs → storage → fill → smoke-test. **1.3.83** Sign Mainstream map. **1.3.84** Sign Canon grammar. **1.3.85** Sign Canon fill. **1.3.86** Sign Canon storage. **1.3.87** Sign Canon materialization. **1.3.88** Planet × Sign smoke-test PASS. **1.3.89** Houses Mainstream map. **1.3.90** House Canon grammar. **1.3.91** House Canon fill. **1.3.92** House Canon storage/materialization. **1.3.93** Planet × House smoke-test PASS. **1.3.94** Mainstream Aspect Semantic Map. **1.3.95** Aspect Canon grammar. **1.3.96** Aspect Canon fill. **1.3.97** Aspect Canon storage/materialization. **1.3.98** stored Planet × Aspect smoke PASS. **1.3.99** Angle Canon model. Дальше Mainstream Angle map, не pair essays. Слои / provenance shape без нового enum. STOP Aspects. STOP Houses. STOP Signs.
...
- **1.3.99 (2026-08-22)** — IL-1 process (no ingest, no object change, no schema): Angle Canon model (parent steps 1–4). ASC/MC = orientation loci (horizon vs meridian). Routing / projection / public–private-as-payload rejected. Named slots unspecified. Next = Mainstream Angle Semantic Map, not fill. STOP Aspects. STOP Houses. STOP Signs.

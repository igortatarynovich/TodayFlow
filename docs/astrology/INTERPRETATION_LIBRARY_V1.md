...
- **SoT after:** live AspectPair frames read `astro.aspect.*.canon.relation`. Four gates PASS. Historical 1.3.82 AspectPair = snapshot. Catalog unchanged. **STOP Aspects.** Next = ASC/MC (`NEED_MODEL`).
- **Public contract changed?** no
- **Migration required?** no
...
**Next named:** ASC/MC definition (parent 1–4). **STOP Aspects.** **STOP Houses.** **STOP Signs.**
...
Не начинать с 10 000 комбинаций. Методология отбора смысла: **1.3.76–1.3.82** Mainstream → grammar → packs → storage → fill → smoke-test. **1.3.83** Sign Mainstream map. **1.3.84** Sign Canon grammar. **1.3.85** Sign Canon fill. **1.3.86** Sign Canon storage. **1.3.87** Sign Canon materialization. **1.3.88** Planet × Sign smoke-test PASS. **1.3.89** Houses Mainstream map. **1.3.90** House Canon grammar. **1.3.91** House Canon fill. **1.3.92** House Canon storage/materialization. **1.3.93** Planet × House smoke-test PASS. **1.3.94** Mainstream Aspect Semantic Map. **1.3.95** Aspect Canon grammar. **1.3.96** Aspect Canon fill. **1.3.97** Aspect Canon storage/materialization. **1.3.98** stored Planet × Aspect smoke PASS. Дальше ASC/MC, не pair essays. Слои / provenance shape без нового enum. STOP Aspects. STOP Houses. STOP Signs.
...
- **1.3.98 (2026-08-22)** — IL-1 diagnostic (no ingest, no object change, no schema): stored Planet × Aspect composition smoke. Four gates PASS from `canon.relation`. Trine vs Sextile on one pair; `interaction=flow` is not the operator. Historical 1.3.82 AspectPair = snapshot. STOP Aspects. Next = ASC/MC. STOP Houses. STOP Signs.

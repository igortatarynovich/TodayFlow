...
**Ingest order (not this pass):** Houlding ontology → Pulse Part One → Cell C winner → Hand Ch.10. Both B and D are readable; readability is not the tie-break. Classification densify first so later-interpretive required slots are not filled from the first open modern author.

**Stopped before step 10.** No 12 sign objects. No schema change. No CORE. No planet coverage hunt.

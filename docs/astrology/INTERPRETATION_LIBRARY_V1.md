...
|------|-------|---------------------|
| Classification | `mode` · `element` · `orientation` (plus commanding/obeying as claims, not extra schema) | Already attested as *systems* with collisions. Schema-required for `type=sign`. Object slot values still wait for a named Architecture impact (Lilly grid as school_specific draft; collisions remain claims). |

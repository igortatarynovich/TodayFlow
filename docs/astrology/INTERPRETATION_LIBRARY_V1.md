...
- **SoT after:** two packs locked. Origin `direct` from 1.3.100 include. Five gates. Collision vs House 1/10. Secondary unused. Catalog untouched. Next = storage/materialization, not objects, not smoke.
- **Public contract changed?** no
- **Migration required?** no
...
**Next named:** Angle Canon fill — **done 1.3.102.** Next = storage/materialization. Then stored Planet×Angle smoke → STOP Angles → final atomic smoke → Knowledge Core V1 FREEZE. After freeze: IL-2. **STOP Aspects.** **STOP Houses.** **STOP Signs.**
...
Two packs. Origin `direct` from 1.3.100 include. ASC `doorway-meeting · how-met · automatic-response`. MC `culmination · outer-mark · aiming`. Five gates. Collision vs House 1/10. Secondary unused. `personal-facing` / `public-facing` leftover include, not a second atom. Dry-run 1.3.101 not inherited. Catalog unchanged.

**Not this pass:** schema · objects · smoke · pretty lemmas.

**Next named:** Angle Canon storage / materialization. Not smoke. Then stored Planet×Angle smoke → STOP Angles → final atomic smoke → Knowledge Core V1 FREEZE. After freeze: IL-2. **STOP Aspects.** **STOP Houses.** **STOP Signs.**
...
Не начинать с 10 000 комбинаций. Методология отбора смысла: **1.3.76–1.3.82** Mainstream → grammar → packs → storage → fill → smoke-test. **1.3.83** Sign Mainstream map. **1.3.84** Sign Canon grammar. **1.3.85** Sign Canon fill. **1.3.86** Sign Canon storage. **1.3.87** Sign Canon materialization. **1.3.88** Planet × Sign smoke-test PASS. **1.3.89** Houses Mainstream map. **1.3.90** House Canon grammar. **1.3.91** House Canon fill. **1.3.92** House Canon storage/materialization. **1.3.93** Planet × House smoke-test PASS. **1.3.94** Mainstream Aspect Semantic Map. **1.3.95** Aspect Canon grammar. **1.3.96** Aspect Canon fill. **1.3.97** Aspect Canon storage/materialization. **1.3.98** stored Planet × Aspect smoke PASS. **1.3.99** Angle Canon model. **1.3.100** Mainstream Angle map. **1.3.101** Angle Canon grammar (`orientation`). **1.3.102** Angle Canon fill. Дальше Angle Canon storage/materialization, не pair essays. Слои / provenance shape без нового enum. STOP Aspects. STOP Houses. STOP Signs.

| ID | Работа | Выход |
|----|--------|--------|
| **IL-0** | Foundation: корпус, evidence, provenance, declared gates | ✅ 2026-08-17 |
| **IL-1** | ~100 surface-neutral objects из корпуса + review | in progress (36 drafts: planets 7 · signs 12 with `canon` manner/excess · houses 12 with `canon` arena · aspects 5 with `canon` relation; Uranus, Neptune and Pluto claims without objects) |
...
- **1.3.102 (2026-08-22)** — IL-1 process (no ingest, no object change, no schema): Angle Canon fill. Two packs. Origin `direct` from 1.3.100 include. Five gates. Collision vs House 1/10. Secondary unused. Next = storage/materialization, not objects, not smoke. STOP Aspects. STOP Houses. STOP Signs.

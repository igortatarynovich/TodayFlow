...
**Next named:** wire calc → IL (not `active`, not Relevance). **STOP Angles.** **STOP Aspects.** **STOP Houses.** **STOP Signs.** Do **not** start CORE scoring. Do **not** start ASC cookbooks. Do **not** start Relevance / Prioritization engines.
...
| **scale** | V1 coverage contract (не каталог пар) | **done 1.3.110**; 616 composed cells; 12 gold candidates; wire named, not live |

Масштаб библиотеки — **done 1.3.110.** Если модель не выдерживает источники — чинить ontology, не плодить объекты и не трогать Today-прозу. Next named = wire calc → IL (не `active`).

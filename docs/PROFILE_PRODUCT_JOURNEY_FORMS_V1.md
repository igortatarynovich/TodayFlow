...
| | |
|---|---|
| Чувство | «Понятно, зачем открыть Today.» |
| Метрика | Мост объясняет продолжение пути — **не** второе «что делать» |

### Одна задача моста

> **Почему теперь имеет смысл открыть Today?**  
> Не: «что делать?» — это только `effort_vector`.  
> Не мотивация и не прогноз дня.

```text
Profile показал устойчивую особенность
        ↓
bridge_line — почему Today естественно продолжает это
        ↓
CTA → Today  (навигация, не совет)
```

| Case | bridge_line (смысл) | CTA |
|------|---------------------|-----|
| A (tension) | Особенность ясна на уровне портрета; Today показывает, как она проявляется в конкретном дне | → Today |
| C (repeat) | Повтор уже назван; Today — следующий экран, где видно проявление и сдвиг | → Today |

Запрет: императив («отметь / сделай»); дубль `effort_vector`; прогноз «завтра»; day stone на Profile (PR-4).
...
| 5 | `bridge_line` — только «зачем Today», не «что делать» | Шаг 5 | **IN CODE** v0.2 — path continuity; forbid imperative / effort duplicate |

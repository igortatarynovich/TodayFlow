...
| **Солнце · стихия · numerology_core** · *шапка* | ≥ дата | Free+ | sun_sign, element, life_path, birthday… | `base_astrology` (+ date numbers in facts) | llm + facts | keys present | нет даты | — (блок с датой) | Опоры **про пользователя** (вклад в ядро), не словарные статьи |
...
| **Нумерология имени** · *шапка, optional* | имя + дата | Free+ | name numbers | `name_numerology` | llm | имя usable | нет имени | «Добавьте имя для разбора имени — на натальную карту не влияет.» | Самопрезентация / мотивация **этого** человека |
